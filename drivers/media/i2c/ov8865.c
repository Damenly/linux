...
static int ov8865_s_ctrl(struct v4l2_ctrl *ctrl)
{
 ...
 if (ctrl->id == V4L2_CID_VBLANK) {
   ...
   exposure_max = sensor->state.mode->output_size_y + ctrl->val - OV8865_INTEGRATION_TIME_MARGIN;
...
static int ov8865_ctrls_init(struct ov8865_sensor *sensor)
{
  ...
  unsigned int hblank;
	...
	ctrls->exposure = v4l2_ctrl_new_std(handler, ops, V4L2_CID_EXPOSURE, 2, 65535, 1,
			  32);

	/* Gain */

	v4l2_ctrl_new_std(handler, ops, V4L2_CID_ANALOGUE_GAIN, 128, 2048, 128, 128);
