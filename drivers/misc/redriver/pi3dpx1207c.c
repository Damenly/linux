...
struct pi3dpx_config redriver_config[2] = {
...

	{
		...
		{0x13 ,0x11 ,0x20 ,0x72 ,0x00 ,0xa7 ,0xa7 ,0xa7 ,0xa7},
	}
                ...
                unsigned long val, void *v)
{
	...
	switch (val) {
		case TYPEC_DP_STATE_E:
			if (*flip) {
				...
				pi3dpx_i2c_write(priv,9,&redriver_config[1].full_dp_config);
			} else {
	...
	case TYPEC_DP_STATE_D:
			if (*flip) {
				pr_err("nihuan redriver flip\n");
				pi3dpx_i2c_write(priv,9,&redriver_config[1].usb_dp_config);
			} else {
				pr_err("nihuan redriver no flip\n");
				pi3dpx_i2c_write(priv,9,&redriver_config[0].usb_dp_config);
			}
		break;
	}
