...
static int esdfs_create(struct mnt_idmap *mnt_userns, struct inode *dir,
...
static int esdfs_mkdir(struct mnt_idmap *mnt_userns, struct inode *dir,
 ...
 */
static int esdfs_rename(struct mnt_idmap *mnt_userns,
			...
			unsigned int flags)
{
	...
	rd.old_mnt_idmap = mnt_userns;
	rd.old_dir = lower_old_dir_dentry->d_inode;
	rd.old_dentry = lower_old_dentry;
	rd.new_mnt_idmap = mnt_userns;
...
static int esdfs_permission(struct mnt_idmap *mnt_userns,
...
static int esdfs_setattr(struct mnt_idmap *mnt_userns,
...
static int esdfs_getattr(struct mnt_idmap *mnt_userns,
			 ...
			 u32 request_mask, unsigned int flags)
{
	...
	generic_fillattr(mnt_userns, request_mask, inode, stat);
