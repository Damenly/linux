...
leaf_item_out_of_bounce_error:
			...
			type = apfs_disk_key_type(disk_key);

			if (APFS_ROOT_ITEM_KEY == type) {
				struct apfs_root_item root_item;
				u32 root_item_offset;
				u64 next_bytenr;

				root_item_offset = item_offset +
					offsetof(struct apfs_leaf, items);
				if (root_item_offset + item_size >
				    sf->block_ctx->len)
					goto leaf_item_out_of_bounce_error;
				apfsic_read_from_block_data(
					sf->block_ctx, &root_item,
					root_item_offset,
					item_size);
				next_bytenr = apfs_root_bytenr(&root_item);

				sf->error =
				    apfsic_create_link_to_next_block(
						state,
						sf->block,
						sf->block_ctx,
						next_bytenr,
						...
						&sf->mirror_num,
						disk_key,
						apfs_root_generation(
						&root_item));
				if (sf->error)
					goto one_stack_frame_backwards;

				if (NULL != sf->next_block) {
					struct apfs_header *const next_hdr =
					    (struct apfs_header *)
					    sf->next_block_ctx.datav[0];

					next_stack =
					    apfsic_stack_frame_alloc();
					if (NULL == next_stack) {
						sf->error = -1;
						apfsic_release_block_ctx(
								&sf->
								next_block_ctx);
						goto one_stack_frame_backwards;
					}
			...
			} else if (APFS_EXTENT_DATA_KEY == type &&
				   state->include_extent_data) {
				sf->error = apfsic_handle_extent_data(
						state,
						sf->block,
						sf->block_ctx,
						item_offset,
		...
		struct apfs_node *const nodehdr = (struct apfs_node *)sf->hdr;

		if (-1 == sf->i) {
			sf->nr = apfs_stack_header_nritems(&nodehdr->header);
...
continue_with_current_node_stack_frame:
		...
		if (sf->i < sf->nr) {
			struct apfs_key_ptr key_ptr;
			u32 key_ptr_offset;
			u64 next_bytenr;

			key_ptr_offset = (uintptr_t)(nodehdr->ptrs + sf->i) -
					  (uintptr_t)nodehdr;
			if (key_ptr_offset + sizeof(struct apfs_key_ptr) >
			    sf->block_ctx->len) {
				pr_info("apfsic: node item out of bounce at logical %llu, dev %s\n",
				       sf->block_ctx->start,
				       sf->block_ctx->dev->name);
				goto one_stack_frame_backwards;
			}
			...
			next_bytenr = apfs_stack_key_blockptr(&key_ptr);

			sf->error = apfsic_create_link_to_next_block(
					state,
					sf->block,
					sf->block_ctx,
					next_bytenr,
					...
					&sf->mirror_num,
					&key_ptr.key,
					apfs_stack_key_generation(&key_ptr));
			if (sf->error)
				goto one_stack_frame_backwards;
			...
			if (NULL != sf->next_block) {
				...
				next_stack = apfsic_stack_frame_alloc();
				if (NULL == next_stack) {
					sf->error = -1;
					goto one_stack_frame_backwards;
				}
...
one_stack_frame_backwards:
	if (NULL != sf->prev) {
		struct apfsic_stack_frame *const prev = sf->prev;

		/* the one for the initial block is freed in the caller */
		apfsic_release_block_ctx(sf->block_ctx);

		if (sf->error) {
			prev->error = sf->error;
			apfsic_stack_frame_free(sf);
			sf = prev;
			goto one_stack_frame_backwards;
		}
...
{
	struct apfs_fs_info *fs_info = state->fs_info;
	struct apfsic_block *next_block = NULL;
	int ret;
	struct apfsic_block_link *l;
	...
	*next_blockp = NULL;
	if (0 == *num_copiesp) {
		*num_copiesp = apfs_num_copies(fs_info, next_bytenr,
						state->metablock_size);
		if (state->print_mask & APFSIC_PRINT_MASK_NUM_COPIES)
			pr_info("num_copies(log_bytenr=%llu) = %d\n",
	...
	if (state->print_mask & APFSIC_PRINT_MASK_VERBOSE)
		pr_info("apfsic_create_link_to_next_block(mirror_num=%d)\n",
		       *mirror_nump);
	ret = apfsic_map_block(state, next_bytenr,
				state->metablock_size,
				next_block_ctx, *mirror_nump);
	if (ret) {
		pr_info("apfsic: apfsic_map_block(@%llu, mirror=%d) failed!\n",
		       next_bytenr, *mirror_nump);
		apfsic_release_block_ctx(next_block_ctx);
		*next_blockp = NULL;
		return -1;
	}

	next_block = apfsic_block_lookup_or_add(state,
	...
	if (NULL == l) {
		...
		if (state->print_mask & APFSIC_PRINT_MASK_VERBOSE)
			apfsic_print_add_link(state, l);

		list_add(&l->node_ref_to, &block->ref_to_list);
		list_add(&l->node_ref_from, &next_block->ref_from_list);

		apfsic_block_link_hashtable_add(l,
						 &state->block_link_hashtable);
	} else {
	...
	if (limit_nesting > 0 && did_alloc_block_link) {
		ret = apfsic_read_block(state, next_block_ctx);
		if (ret < (int)next_block_ctx->len) {
			pr_info("apfsic: read block @logical %llu failed!\n",
			       next_bytenr);
			apfsic_release_block_ctx(next_block_ctx);
			*next_blockp = NULL;
			return -1;
...
static int apfsic_handle_extent_data(
		struct apfsic_state *state,
		struct apfsic_block *block,
		struct apfsic_block_data_ctx *block_ctx,
		u32 item_offset, int force_iodone_flag)
{
	struct apfs_fs_info *fs_info = state->fs_info;
	struct apfs_file_extent_item file_extent_item;
	...
	file_extent_item_offset = offsetof(struct apfs_leaf, items) +
				  item_offset;
	if (file_extent_item_offset +
	    offsetof(struct apfs_file_extent_item, disk_num_bytes) >
	    block_ctx->len) {
		pr_info("apfsic: file item out of bounce at logical %llu, dev %s\n",
		       block_ctx->start, block_ctx->dev->name);
		return -1;
	}

	apfsic_read_from_block_data(block_ctx, &file_extent_item,
		file_extent_item_offset,
		offsetof(struct apfs_file_extent_item, disk_num_bytes));
	if (APFS_FILE_EXTENT_REG != file_extent_item.type ||
	    apfs_stack_file_extent_disk_bytenr(&file_extent_item) == 0) {
		if (state->print_mask & APFSIC_PRINT_MASK_VERY_VERBOSE)
			pr_info("extent_data: type %u, disk_bytenr = %llu\n",
			       file_extent_item.type,
			       apfs_stack_file_extent_disk_bytenr(
			       &file_extent_item));
		return 0;
	}

	if (file_extent_item_offset + sizeof(struct apfs_file_extent_item) >
	    block_ctx->len) {
		pr_info("apfsic: file item out of bounce at logical %llu, dev %s\n",
		       block_ctx->start, block_ctx->dev->name);
		return -1;
	...
	next_bytenr = apfs_stack_file_extent_disk_bytenr(&file_extent_item);
	if (apfs_stack_file_extent_compression(&file_extent_item) ==
	    APFS_COMPRESS_NONE) {
		next_bytenr += apfs_stack_file_extent_offset(&file_extent_item);
		num_bytes = apfs_stack_file_extent_num_bytes(&file_extent_item);
	} else {
		num_bytes = apfs_stack_file_extent_disk_num_bytes(&file_extent_item);
	}
	generation = apfs_stack_file_extent_generation(&file_extent_item);

	if (state->print_mask & APFSIC_PRINT_MASK_VERY_VERBOSE)
		pr_info("extent_data: type %u, disk_bytenr = %llu, offset = %llu, num_bytes = %llu\n",
		       file_extent_item.type,
		       apfs_stack_file_extent_disk_bytenr(&file_extent_item),
		       apfs_stack_file_extent_offset(&file_extent_item),
		       num_bytes);
	while (num_bytes > 0) {
		u32 chunk_len;
		int num_copies;
		...
		for (mirror_num = 1; mirror_num <= num_copies; mirror_num++) {
			...
			if (state->print_mask & APFSIC_PRINT_MASK_VERBOSE)
				pr_info("apfsic_handle_extent_data(mirror_num=%d)\n",
					mirror_num);
			if (state->print_mask & APFSIC_PRINT_MASK_VERY_VERBOSE)
			...
			l = apfsic_block_link_lookup_or_add(state,
							     &next_block_ctx,
							     next_block, block,
							     generation);
			apfsic_release_block_ctx(&next_block_ctx);
			     ...
			     int mirror_num)
{
	...
	int ret;
	u64 length;
	struct apfs_bio *multi = NULL;
	struct apfs_device *device;

	length = len;
	ret = apfs_map_block(fs_info, APFS_MAP_READ,
	...
	if (test_bit(APFS_DEV_STATE_MISSING, &device->dev_state) ||
		...
		block_ctx_out->dev = apfsic_dev_state_lookup(
							device->bdev->bd_dev);
	...
	if (NULL == block_ctx_out->dev) {
		ret = -ENXIO;
		pr_info("apfsic: error, cannot lookup dev (#1)!\n");
	}
			      ...
			      struct apfsic_block_data_ctx *block_ctx)
{
	...
	if (!PAGE_ALIGNED(block_ctx->dev_bytenr)) {
		pr_info("apfsic: read_block() with unaligned bytenr %llu\n",
		       block_ctx->dev_bytenr);
		return -1;
	}
	...
	for (i = 0; i < num_pages;) {
		...
		bio = apfs_io_bio_alloc(num_pages - i);
		...
		if (j == i) {
			pr_info("apfsic: error, failed to add a single page!\n");
			return -1;
		}
		if (submit_bio_wait(bio)) {
			pr_info("apfsic: read error at logical %llu dev %s!\n",
			       block_ctx->start, block_ctx->dev->name);
			bio_put(bio);
			return -1;
		}
...
static void apfsic_dump_database(struct apfsic_state *state)
{
	const struct apfsic_block *b_all;
	...
	list_for_each_entry(b_all, &state->all_blocks_list, all_blocks_node) {
		...
		pr_info("%c-block @%llu (%s/%llu/%d)\n",
		       apfsic_get_block_type(state, b_all),
		...
		list_for_each_entry(l, &b_all->ref_to_list, node_ref_to) {
			pr_info(" %c @%llu (%s/%llu/%d) refers %u* to %c @%llu (%s/%llu/%d)\n",
			       ...
			       b_all->dev_bytenr, b_all->mirror_num,
			       l->ref_cnt,
			       apfsic_get_block_type(state, l->block_ref_to),
		...
		list_for_each_entry(l, &b_all->ref_from_list, node_ref_from) {
			pr_info(" %c @%llu (%s/%llu/%d) is ref %u* from %c @%llu (%s/%llu/%d)\n",
			       ...
			       b_all->dev_bytenr, b_all->mirror_num,
			       l->ref_cnt,
			       apfsic_get_block_type(state, l->block_ref_from),
 ...
 */
static noinline_for_stack int apfsic_test_for_metadata(
		struct apfsic_state *state,
		char **datav, unsigned int num_pages)
{
	struct apfs_fs_info *fs_info = state->fs_info;
	SHASH_DESC_ON_STACK(shash, fs_info->csum_shash);
	struct apfs_header *h;
	...
	h = (struct apfs_header *)datav[0];

	if (memcmp(h->fsid, fs_info->fs_devices->fsid, APFS_FSID_SIZE))
		return 1;
	...
	for (i = 0; i < num_pages; i++) {
		u8 *data = i ? datav[i] : (datav[i] + APFS_CSUM_SIZE);
		size_t sublen = i ? PAGE_SIZE :
				    (PAGE_SIZE - APFS_CSUM_SIZE);
					  ...
					  int submit_bio_bh_rw)
{
	...
	struct apfsic_block_data_ctx block_ctx;
	int ret;
	struct apfsic_state *state = dev_state->state;
...
again:
	...
	processed_len = 0;
	is_metadata = (0 == apfsic_test_for_metadata(state, mapped_datav,
						      num_pages));

	block = apfsic_block_hashtable_lookup(bdev, dev_bytenr,
					       &state->block_hashtable);
	if (NULL != block) {
		u64 bytenr = 0;
		struct apfsic_block_link *l, *tmp;

		if (block->is_superblock) {
			bytenr = apfs_super_bytenr((struct apfs_super_block *)
						    mapped_datav[0]);
			if (num_pages * PAGE_SIZE <
			    APFS_SUPER_INFO_SIZE) {
				pr_info("apfsic: cannot work with too short bios!\n");
				return;
			}
			is_metadata = 1;
			BUG_ON(!PAGE_ALIGNED(APFS_SUPER_INFO_SIZE));
			processed_len = APFS_SUPER_INFO_SIZE;
			if (state->print_mask &
			    APFSIC_PRINT_MASK_TREE_BEFORE_SB_WRITE) {
				pr_info("[before new superblock is written]:\n");
				apfsic_dump_tree_sub(state, block, 0);
			}
		}
		if (is_metadata) {
			if (!block->is_superblock) {
				...
				bytenr = apfs_stack_header_bytenr(
						(struct apfs_header *)
						mapped_datav[0]);
				apfsic_cmp_log_and_dev_bytenr(state, bytenr,
							       dev_state,
							       dev_bytenr);
			}
			if (state->print_mask & APFSIC_PRINT_MASK_VERBOSE) {
			...
			if (state->print_mask & APFSIC_PRINT_MASK_VERBOSE)
				pr_info("Written block @%llu (%s/%llu/%d) found in hash table, %c.\n",
				       ...
				       block->mirror_num,
				       apfsic_get_block_type(state, block));
		}
		...
		if (apfsic_is_block_ref_by_superblock(state, block, 0)) {
			pr_info("apfs: attempt to overwrite %c-block @%llu (%s/%llu/%d), old(gen=%llu, objectid=%llu, type=%d, offset=%llu), new(gen=%llu), which is referenced by most recent superblock (superblockgen=%llu)!\n",
			       apfsic_get_block_type(state, block), bytenr,
			       dev_state->name, dev_bytenr, block->mirror_num,
			       block->generation,
			       apfs_disk_key_objectid(&block->disk_key),
			       apfs_disk_key_type(&block->disk_key),
			       apfs_disk_key_offset(&block->disk_key),
			       apfs_stack_header_generation(
				       (struct apfs_header *) mapped_datav[0]),
			       state->max_superblock_generation);
			apfsic_dump_tree(state);
		}

		if (!block->is_iodone && !block->never_written) {
			pr_info("apfs: attempt to overwrite %c-block @%llu (%s/%llu/%d), oldgen=%llu, newgen=%llu, which is not yet iodone!\n",
			       apfsic_get_block_type(state, block), bytenr,
			       dev_state->name, dev_bytenr, block->mirror_num,
			       block->generation,
			       apfs_stack_header_generation(
		...
		list_for_each_entry_safe(l, tmp, &block->ref_to_list,
					 node_ref_to) {
			if (state->print_mask & APFSIC_PRINT_MASK_VERBOSE)
				apfsic_print_rem_link(state, l);
			l->ref_cnt--;
			if (0 == l->ref_cnt) {
				list_del(&l->node_ref_to);
				list_del(&l->node_ref_from);
				apfsic_block_link_hashtable_remove(l);
				apfsic_block_link_free(l);
			}
		}
		...
		if (is_metadata || state->include_extent_data) {
			...
			if (NULL != bio) {
				...
				if (!*bio_is_patched) {
					...
					bio->bi_end_io = apfsic_bio_end_io;
					*bio_is_patched = 1;
				} else {
					struct apfsic_block *chained_block =
					    (struct apfsic_block *)
					    bio->bi_private;
		...
		if (is_metadata) {
			...
			if (block->is_superblock) {
				BUG_ON(PAGE_SIZE !=
				       APFS_SUPER_INFO_SIZE);
				ret = apfsic_process_written_superblock(
						state,
						block,
						(struct apfs_super_block *)
						mapped_datav[0]);
				if (state->print_mask &
				    APFSIC_PRINT_MASK_TREE_AFTER_SB_WRITE) {
					pr_info("[after new superblock is written]:\n");
					apfsic_dump_tree_sub(state, block, 0);
				}
			} else {
				block->mirror_num = 0;	/* unknown */
				ret = apfsic_process_metablock(
						state,
						block,
						&block_ctx,
						0, 0);
			...
			if (ret)
				pr_info("apfsic: apfsic_process_metablock(root @%llu) failed!\n",
				       dev_bytenr);
		} else {
			block->is_metadata = 0;
			block->mirror_num = 0;	/* unknown */
			block->generation = APFSIC_GENERATION_UNKNOWN;
			if (!state->include_extent_data
				 ...
				 */
				apfsic_block_hashtable_remove(block);
				list_del(&block->all_blocks_node);
				apfsic_block_free(block);
			}
		}
		apfsic_release_block_ctx(&block_ctx);
	} else {
		...
		if (!is_metadata) {
			...
			bytenr = apfs_stack_header_bytenr(
					(struct apfs_header *)
					mapped_datav[0]);
			apfsic_cmp_log_and_dev_bytenr(state, bytenr, dev_state,
						       dev_bytenr);
			if (state->print_mask & APFSIC_PRINT_MASK_VERBOSE)
		...
		apfsic_block_hashtable_add(block, &state->block_hashtable);

		if (is_metadata) {
			ret = apfsic_process_metablock(state, block,
							&block_ctx, 0, 0);
			if (ret)
				pr_info("apfsic: process_metablock(root @%llu) failed!\n",
				       dev_bytenr);
		}
		apfsic_release_block_ctx(&block_ctx);
	}
...
static void apfsic_bio_end_io(struct bio *bp)
{
	struct apfsic_block *block = (struct apfsic_block *)bp->bi_private;
	int iodone_w_error;
	...
	do {
		struct apfsic_block *next_block;
		struct apfsic_dev_state *const dev_state = block->dev_state;

		if ((dev_state->state->print_mask &
		     APFSIC_PRINT_MASK_END_IO_BIO_BH))
			pr_info("bio_end_io(err=%d) for %c @%llu (%s/%llu/%d)\n",
			       bp->bi_status,
			       apfsic_get_block_type(dev_state->state, block),
...
static int apfsic_process_written_superblock(
		struct apfsic_state *state,
		struct apfsic_block *const superblock,
		struct apfs_super_block *const super_hdr)
{
	struct apfs_fs_info *fs_info = state->fs_info;
	int pass;

	superblock->generation = apfs_super_generation(super_hdr);
	if (!(superblock->generation > state->max_superblock_generation ||
	      0 == state->max_superblock_generation)) {
		if (state->print_mask & APFSIC_PRINT_MASK_SUPERBLOCK_WRITE)
			pr_info("apfsic: superblock @%llu (%s/%llu/%d) with old gen %llu <= %llu\n",
			       superblock->logical_bytenr,
			       superblock->dev_state->name,
			       superblock->dev_bytenr, superblock->mirror_num,
			       apfs_super_generation(super_hdr),
			       state->max_superblock_generation);
	} else {
		if (state->print_mask & APFSIC_PRINT_MASK_SUPERBLOCK_WRITE)
			pr_info("apfsic: got new superblock @%llu (%s/%llu/%d) with new gen %llu > %llu\n",
			       superblock->logical_bytenr,
			       superblock->dev_state->name,
			       superblock->dev_bytenr, superblock->mirror_num,
			       apfs_super_generation(super_hdr),
	...
	for (pass = 0; pass < 3; pass++) {
		...
		int mirror_num;
		const char *additional_string = NULL;
		struct apfs_disk_key tmp_disk_key = {0};

		apfs_set_disk_key_objectid(&tmp_disk_key,
					    APFS_ROOT_ITEM_KEY);
		apfs_set_disk_key_objectid(&tmp_disk_key, 0);
		...
		case 0:
			...
			next_bytenr = apfs_super_root(super_hdr);
			if (state->print_mask &
			    APFSIC_PRINT_MASK_ROOT_CHUNK_LOG_TREE_LOCATION)
				pr_info("root@%llu\n", next_bytenr);
			break;
		case 1:
			...
			next_bytenr = apfs_super_chunk_root(super_hdr);
			if (state->print_mask &
			    APFSIC_PRINT_MASK_ROOT_CHUNK_LOG_TREE_LOCATION)
				pr_info("chunk@%llu\n", next_bytenr);
			break;
		case 2:
			...
			next_bytenr = apfs_super_log_root(super_hdr);
			if (0 == next_bytenr)
				continue;
			if (state->print_mask &
			    APFSIC_PRINT_MASK_ROOT_CHUNK_LOG_TREE_LOCATION)
		...
		for (mirror_num = 1; mirror_num <= num_copies; mirror_num++) {
			...
			if (state->print_mask & APFSIC_PRINT_MASK_VERBOSE)
				pr_info("apfsic_process_written_superblock(mirror_num=%d)\n", mirror_num);
			ret = apfsic_map_block(state, next_bytenr,
						APFS_SUPER_INFO_SIZE,
						&tmp_next_block_ctx,
						mirror_num);
	...
	if (WARN_ON(-1 == apfsic_check_all_ref_blocks(state, superblock, 0)))
		apfsic_dump_tree(state);
					...
					int recursion_level)
{
	...
	if (recursion_level >= 3 + APFS_MAX_LEVEL) {
		/*
		 ...
		 */
		if (state->print_mask & APFSIC_PRINT_MASK_VERBOSE)
			pr_info("apfsic: abort cyclic linkage (case 1).\n");
	...
	list_for_each_entry(l, &block->ref_to_list, node_ref_to) {
		...
		if (l->block_ref_to->never_written) {
			pr_info("apfs: attempt to write superblock which references block %c @%llu (%s/%llu/%d) which is never written!\n",
			       apfsic_get_block_type(state, l->block_ref_to),
			       l->block_ref_to->logical_bytenr,
			       l->block_ref_to->dev_state->name,
			       l->block_ref_to->dev_bytenr,
			...
			pr_info("apfs: attempt to write superblock which references block %c @%llu (%s/%llu/%d) which is not yet iodone!\n",
			       apfsic_get_block_type(state, l->block_ref_to),
			       l->block_ref_to->logical_bytenr,
			       l->block_ref_to->dev_state->name,
			       l->block_ref_to->dev_bytenr,
			...
			pr_info("apfs: attempt to write superblock which references block %c @%llu (%s/%llu/%d) which has write error!\n",
			       apfsic_get_block_type(state, l->block_ref_to),
			       l->block_ref_to->logical_bytenr,
			       l->block_ref_to->dev_state->name,
			       l->block_ref_to->dev_bytenr,
			...
			ret = -1;
			   ...
			   APFSIC_GENERATION_UNKNOWN !=
			   l->block_ref_to->generation) {
			pr_info("apfs: attempt to write superblock which references block %c @%llu (%s/%llu/%d) with generation %llu != parent generation %llu!\n",
			       apfsic_get_block_type(state, l->block_ref_to),
			       l->block_ref_to->logical_bytenr,
			       l->block_ref_to->dev_state->name,
			       l->block_ref_to->dev_bytenr,
			...
			pr_info("apfs: attempt to write superblock which references block %c @%llu (%s/%llu/%d) which is not flushed out of disk's write cache (block flush_gen=%llu, dev->flush_gen=%llu)!\n",
			       apfsic_get_block_type(state, l->block_ref_to),
			       l->block_ref_to->logical_bytenr,
			       l->block_ref_to->dev_state->name,
			       l->block_ref_to->dev_bytenr,
			       ...
			       l->block_ref_to->dev_state->last_flush_gen);
			ret = -1;
		} else if (-1 == apfsic_check_all_ref_blocks(state,
							      l->block_ref_to,
							      recursion_level +
							      1)) {
			ret = -1;
...
static int apfsic_is_block_ref_by_superblock(
		const struct apfsic_state *state,
		const struct apfsic_block *block,
		int recursion_level)
{
	const struct apfsic_block_link *l;
	...
	if (recursion_level >= 3 + APFS_MAX_LEVEL) {
		...
		if (state->print_mask & APFSIC_PRINT_MASK_VERBOSE)
			pr_info("apfsic: abort cyclic linkage (case 2).\n");
	...
	list_for_each_entry(l, &block->ref_from_list, node_ref_from) {
		...
		if (l->block_ref_from->is_superblock &&
			...
			return 1;
		else if (apfsic_is_block_ref_by_superblock(state,
							    l->block_ref_from,
							    recursion_level +
							    1))
			return 1;
...
static void apfsic_dump_tree(const struct apfsic_state *state)
{
	apfsic_dump_tree_sub(state, state->latest_superblock, 0);
}
				  ...
				  int indent_level)
{
	...
	indent_add = sprintf(buf, "%c-%llu(%s/%llu/%u)",
			     ...
			     block->logical_bytenr, block->dev_state->name,
			     block->dev_bytenr, block->mirror_num);
	if (indent_level + indent_add > APFSIC_TREE_DUMP_MAX_INDENT_LEVEL) {
		trace_printk("[...]\n");
		return;
	}
	trace_printk(buf);
	...
	if (block->mirror_num > 1 &&
	    !(state->print_mask & APFSIC_PRINT_MASK_TREE_WITH_ALL_MIRRORS)) {
		trace_printk(" [...]\n");
		return;
	}
	...
	list_for_each_entry(l, &block->ref_to_list, node_ref_to) {
		...
		if (indent_level + indent_add >
		    APFSIC_TREE_DUMP_MAX_INDENT_LEVEL) {
			trace_printk("[...]\n");
			cursor_position = 0;
			continue;
		}
		...
		apfsic_dump_tree_sub(state, l->block_ref_to,
				      indent_level + indent_add);
		cursor_position = 0;
	}
}
		...
		u64 parent_generation)
{
	...
	l = apfsic_block_link_hashtable_lookup(next_block_ctx->dev->bdev,
						next_block_ctx->dev_bytenr,
		...
		int *was_created)
{
	...
	block = apfsic_block_hashtable_lookup(block_ctx->dev->bdev,
					       block_ctx->dev_bytenr,
					       &state->block_hashtable);
	if (NULL == block) {
		struct apfsic_dev_state *dev_state;
		...
		dev_state = apfsic_dev_state_lookup(block_ctx->dev->bdev->bd_dev);
		if (NULL == dev_state) {
			pr_info("apfsic: error, lookup dev_state failed!\n");
			apfsic_block_free(block);
			return NULL;
		}
		block->dev_state = dev_state;
					   ...
					   u64 dev_bytenr)
{
	...
	num_copies = apfs_num_copies(fs_info, bytenr, state->metablock_size);

	for (mirror_num = 1; mirror_num <= num_copies; mirror_num++) {
		ret = apfsic_map_block(state, bytenr, state->metablock_size,
					&block_ctx, mirror_num);
		if (ret) {
			pr_info("apfsic: apfsic_map_block(logical @%llu, mirror %d) failed!\n",
			       bytenr, mirror_num);
			continue;
		}
	...
	if (WARN_ON(!match)) {
		pr_info("apfs: attempt to write M-block which contains logical bytenr that doesn't map to dev+physical bytenr of submit_bio, buffer->log_bytenr=%llu, submit_bio(bdev=%s, phys_bytenr=%llu)!\n",
		       bytenr, dev_state->name, dev_bytenr);
		for (mirror_num = 1; mirror_num <= num_copies; mirror_num++) {
			ret = apfsic_map_block(state, bytenr,
						state->metablock_size,
						&block_ctx, mirror_num);
			if (ret)
				continue;
...
static struct apfsic_dev_state *apfsic_dev_state_lookup(dev_t dev)
{
	return apfsic_dev_state_hashtable_lookup(dev,
						  &apfsic_dev_state_hashtable);
}

static void __apfsic_submit_bio(struct bio *bio)
{
	struct apfsic_dev_state *dev_state;

	if (!apfsic_is_initialized)
		return;

	mutex_lock(&apfsic_mutex);
	/* since apfsic_submit_bio() is also called before
	 * apfsic_mount(), this might return NULL */
	dev_state = apfsic_dev_state_lookup(bio->bi_bdev->bd_dev);
	if (NULL != dev_state &&
		...
		bio_is_patched = 0;
		if (dev_state->state->print_mask &
		    APFSIC_PRINT_MASK_SUBMIT_BIO_BH)
		...
		bio_for_each_segment(bvec, bio, iter) {
			...
			if (dev_state->state->print_mask &
			    APFSIC_PRINT_MASK_SUBMIT_BIO_BH_VERBOSE)
				pr_info("#%u: bytenr=%llu, len=%u, offset=%u\n",
				       i, cur_bytenr, bvec.bv_len, bvec.bv_offset);
			cur_bytenr += bvec.bv_len;
		}
		apfsic_process_written_block(dev_state, dev_bytenr,
					      mapped_datav, segs,
					      bio, &bio_is_patched,
					      bio->bi_opf);
		...
		if (!dev_state->dummy_block_for_bio_bh_flush.is_iodone) {
			if ((dev_state->state->print_mask &
			     (APFSIC_PRINT_MASK_SUBMIT_BIO_BH |
			      APFSIC_PRINT_MASK_VERBOSE)))
				pr_info("apfsic_submit_bio(%s) with FLUSH but dummy block already in use (ignored)!\n",
				       dev_state->name);
		} else {
			struct apfsic_block *const block =
...
leave:
	mutex_unlock(&apfsic_mutex);
}

void apfsic_submit_bio(struct bio *bio)
{
	__apfsic_submit_bio(bio);

	trace_printk(
		"apfs_map_bio: rw %d 0x%x, sector=%llu, size=%u",
		  ...
		  int including_extent_data, u32 print_mask)
{
	...
	if (!PAGE_ALIGNED(fs_info->nodesize)) {
		pr_info("apfsic: cannot handle nodesize %d not being a multiple of PAGE_SIZE %ld!\n",
		       fs_info->nodesize, PAGE_SIZE);
		return -1;
	}
	if (!PAGE_ALIGNED(fs_info->sectorsize)) {
		pr_info("apfsic: cannot handle sectorsize %d not being a multiple of PAGE_SIZE %ld!\n",
	...
	if (!apfsic_is_initialized) {
		mutex_init(&apfsic_mutex);
		apfsic_dev_state_hashtable_init(&apfsic_dev_state_hashtable);
		apfsic_is_initialized = 1;
	}
	mutex_lock(&apfsic_mutex);
	...
	apfsic_block_hashtable_init(&state->block_hashtable);
	apfsic_block_link_hashtable_init(&state->block_link_hashtable);
	...
	list_for_each_entry(device, dev_head, dev_list) {
		...
		apfsic_dev_state_hashtable_add(ds,
						&apfsic_dev_state_hashtable);
	}

	ret = apfsic_process_superblock(state, fs_devices);
	if (0 != ret) {
		mutex_unlock(&apfsic_mutex);
		apfsic_unmount(fs_devices);
		return ret;
	}

	if (state->print_mask & APFSIC_PRINT_MASK_INITIAL_DATABASE)
		apfsic_dump_database(state);
	if (state->print_mask & APFSIC_PRINT_MASK_INITIAL_TREE)
		apfsic_dump_tree(state);
...
{
	struct apfsic_block *b_all, *tmp_all;
	struct apfsic_state *state;
	struct list_head *dev_head = &fs_devices->devices;
	struct apfs_device *device;
	...
	list_for_each_entry(device, dev_head, dev_list) {
		...
		ds = apfsic_dev_state_hashtable_lookup(
				device->bdev->bd_dev,
				&apfsic_dev_state_hashtable);
		if (NULL != ds) {
			state = ds->state;
			apfsic_dev_state_hashtable_remove(ds);
			apfsic_dev_state_free(ds);
		}
	}

	if (NULL == state) {
		pr_info("apfsic: error, cannot find state information on umount!\n");
		mutex_unlock(&apfsic_mutex);
		return;
	}
	...
	list_for_each_entry_safe(b_all, tmp_all, &state->all_blocks_list,
		...
		if (b_all->is_iodone || b_all->never_written)
			apfsic_block_free(b_all);
		else
			pr_info("apfs: attempt to free %c-block @%llu (%s/%llu/%d) on umount which is not yet iodone!\n",
			       apfsic_get_block_type(state, b_all),
			       b_all->logical_bytenr, b_all->dev_state->name,
			       b_all->dev_bytenr, b_all->mirror_num);
	}
