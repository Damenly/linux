...
out:
	...
	spin_lock(&ctl->tree_lock);
	apfs_discard_update_discardable(block_group);
	spin_unlock(&ctl->tree_lock);
	iput(inode);
	return ret;
}
...
int write_cache_extent_entries(struct apfs_io_ctl *io_ctl,
			      struct apfs_free_space_ctl *ctl,
			      struct apfs_block_group *block_group,
			      ...
			      struct list_head *bitmap_list)
{
	...
	struct apfs_free_cluster *cluster_locked = NULL;
	struct rb_node *node = rb_first(&ctl->free_space_offset);
	struct apfs_trim_range *trim_entry;
	...
	while (node) {
		struct apfs_free_space *e;

		e = rb_entry(node, struct apfs_free_space, offset_index);
		*entries += 1;
...
update_cache_item(struct apfs_trans_handle *trans,
		  ...
		  int entries, int bitmaps)
{
	...
	if (ret < 0) {
		clear_extent_bit(&APFS_I(inode)->io_tree, 0, inode->i_size - 1,
	...
	if (ret > 0) {
		...
		if (found_key.objectid != APFS_FREE_SPACE_OBJECTID ||
		    found_key.offset != offset) {
			clear_extent_bit(&APFS_I(inode)->io_tree, 0,
	...
	header = apfs_item_ptr(leaf, path->slots[0],
				struct apfs_free_space_header);
	apfs_set_free_space_entries(leaf, header, entries);
	apfs_set_free_space_bitmaps(leaf, header, bitmaps);
	apfs_set_free_space_generation(leaf, header, trans->transid);
	apfs_mark_buffer_dirty(leaf);
	apfs_release_path(path);
...
static noinline_for_stack int
write_bitmap_entries(struct apfs_io_ctl *io_ctl, struct list_head *bitmap_list)
{
	struct apfs_free_space *entry, *next;
	int ret;
...
{
	io_ctl_drop_pages(io_ctl);
	unlock_extent_cached(&APFS_I(inode)->io_tree, 0,
			     i_size_read(inode) - 1, cached_state);
}

static int __apfs_wait_cache_io(struct apfs_root *root,
				 struct apfs_trans_handle *trans,
				 struct apfs_block_group *block_group,
				 struct apfs_io_ctl *io_ctl,
				 struct apfs_path *path, u64 offset)
...
out:
	...
	if (block_group) {
		...
		if (!ret && list_empty(&block_group->dirty_list))
			...
			block_group->disk_cache_state = APFS_DC_ERROR;
...
int apfs_wait_cache_io(struct apfs_trans_handle *trans,
			...
			struct apfs_path *path)
{
	return __apfs_wait_cache_io(block_group->fs_info->tree_root, trans,
 ...
 */
static int __apfs_write_out_cache(struct apfs_root *root, struct inode *inode,
				   struct apfs_free_space_ctl *ctl,
				   struct apfs_block_group *block_group,
				   struct apfs_io_ctl *io_ctl,
				   struct apfs_trans_handle *trans)
{
	...
	if (block_group && (block_group->flags & APFS_BLOCK_GROUP_DATA)) {
	...
	lock_extent_bits(&APFS_I(inode)->io_tree, 0, i_size_read(inode) - 1,
			 &cached_state);
	...
	ret = apfs_dirty_pages(APFS_I(inode), io_ctl->pages,
	...
	ret = apfs_fdatawrite_range(inode, 0, (u64)-1);
	if (ret)
		goto out;
			  ...
			  struct apfs_path *path)
{
	...
	spin_lock(&block_group->lock);
	if (block_group->disk_cache_state < APFS_DC_SETUP) {
		spin_unlock(&block_group->lock);
		return 0;
	}
	spin_unlock(&block_group->lock);
	...
	ret = __apfs_write_out_cache(fs_info->tree_root, inode, ctl,
	...
	/*
	 * if ret == 0 the caller is expected to call apfs_wait_cache_io
...
static inline u64 offset_to_bitmap(struct apfs_free_space_ctl *ctl,
				   u64 offset)
{
	u64 bitmap_start;
			      ...
			      struct rb_node *node, int bitmap)
{
	...
	while (*p) {
		parent = *p;
		info = rb_entry(parent, struct apfs_free_space, offset_index);
 ...
 * offset.
 */
static struct apfs_free_space *
tree_search_offset(struct apfs_free_space_ctl *ctl,
		   u64 offset, int bitmap_only, int fuzzy)
{
	struct rb_node *n = ctl->free_space_offset.rb_node;
	struct apfs_free_space *entry, *prev = NULL;
	...
	while (1) {
		...
		entry = rb_entry(n, struct apfs_free_space, offset_index);
		prev = entry;
	...
	if (bitmap_only) {
		...
		if (entry->bitmap) {
			...
			if (n) {
				prev = rb_entry(n, struct apfs_free_space,
						offset_index);
				if (!prev->bitmap &&
	...
	if (entry->offset > offset) {
		n = rb_prev(&entry->offset_index);
		if (n) {
			entry = rb_entry(n, struct apfs_free_space,
					offset_index);
...
static inline void
__unlink_free_space(struct apfs_free_space_ctl *ctl,
		    struct apfs_free_space *info)
{
	rb_erase(&info->offset_index, &ctl->free_space_offset);
	ctl->free_extents--;

	if (!info->bitmap && !apfs_free_space_trimmed(info)) {
		ctl->discardable_extents[APFS_STAT_CURR]--;
		ctl->discardable_bytes[APFS_STAT_CURR] -= info->bytes;
	}
}
...
static inline void __bitmap_clear_bits(struct apfs_free_space_ctl *ctl,
				       struct apfs_free_space *info,
				       u64 offset, u64 bytes)
{
	...
	if (!apfs_free_space_trimmed(info)) {
		ctl->discardable_extents[APFS_STAT_CURR] += extent_delta;
		ctl->discardable_bytes[APFS_STAT_CURR] -= bytes;
	}
}
...
static void bitmap_set_bits(struct apfs_free_space_ctl *ctl,
			    struct apfs_free_space *info, u64 offset,
			    u64 bytes)
{
	unsigned long start, count, end;
...
static inline u64 get_max_extent_size(struct apfs_free_space *entry)
{
	if (entry->bitmap)
...
static struct apfs_free_space *
find_free_space(struct apfs_free_space_ctl *ctl, u64 *offset, u64 *bytes,
		unsigned long align, u64 *max_extent_size)
{
	...
	for (node = &entry->offset_index; node; node = rb_next(node)) {
		entry = rb_entry(node, struct apfs_free_space, offset_index);
...
static void add_new_bitmap(struct apfs_free_space_ctl *ctl,
			   struct apfs_free_space *info, u64 offset)
{
			...
			struct apfs_free_space *bitmap_info)
{
	 ...
	 * via __apfs_remove_free_space_cache(), then it may not be freed and
	 * we may leave stats on the table.
	 */
	if (bitmap_info->bytes && !apfs_free_space_trimmed(bitmap_info)) {
		ctl->discardable_extents[APFS_STAT_CURR] -=
			bitmap_info->bitmap_extents;
		ctl->discardable_bytes[APFS_STAT_CURR] -= bitmap_info->bytes;

	}
	unlink_free_space(ctl, bitmap_info);
	kmem_cache_free(apfs_free_space_bitmap_cachep, bitmap_info->bitmap);
	kmem_cache_free(apfs_free_space_cachep, bitmap_info);
	ctl->total_bitmaps--;
	recalculate_thresholds(ctl);
}

static noinline int remove_from_bitmap(struct apfs_free_space_ctl *ctl,
			      struct apfs_free_space *bitmap_info,
			      u64 *offset, u64 *bytes)
{
	u64 end;
...
again:
	...
	*bytes -= search_bytes;
	...
	if (*bytes) {
		...
		bitmap_info = rb_entry(next, struct apfs_free_space,
				       offset_index);
...
static u64 add_bytes_to_bitmap(struct apfs_free_space_ctl *ctl,
			       struct apfs_free_space *info, u64 offset,
			       u64 bytes, enum apfs_trim_state trim_state)
{
	u64 bytes_to_set = 0;
	u64 end;
	 ...
	 */
	if (trim_state == APFS_TRIM_STATE_UNTRIMMED) {
		if (apfs_free_space_trimmed(info)) {
			ctl->discardable_extents[APFS_STAT_CURR] +=
				info->bitmap_extents;
			ctl->discardable_bytes[APFS_STAT_CURR] += info->bytes;
		}
		info->trim_state = APFS_TRIM_STATE_UNTRIMMED;
	}
...
static bool use_bitmap(struct apfs_free_space_ctl *ctl,
		      struct apfs_free_space *info)
{
	struct apfs_block_group *block_group = ctl->private;
	struct apfs_fs_info *fs_info = block_group->fs_info;
...
static const struct apfs_free_space_op free_space_op = {
	.use_bitmap		= use_bitmap,
};

static int insert_into_bitmap(struct apfs_free_space_ctl *ctl,
			      struct apfs_free_space *info)
{
	struct apfs_free_space *bitmap_info;
...
new_bitmap:
	if (info && info->bitmap) {
		...
		if (!info) {
			info = kmem_cache_zalloc(apfs_free_space_cachep,
						 GFP_NOFS);
			if (!info) {
				spin_lock(&ctl->tree_lock);
				ret = -ENOMEM;
		...
		/* allocate the bitmap */
		info->bitmap = kmem_cache_zalloc(apfs_free_space_bitmap_cachep,
						 GFP_NOFS);
		info->trim_state = APFS_TRIM_STATE_TRIMMED;
		spin_lock(&ctl->tree_lock);
		if (!info->bitmap) {
			ret = -ENOMEM;
			goto out;
...
out:
	if (info) {
		if (info->bitmap)
			kmem_cache_free(apfs_free_space_bitmap_cachep,
					info->bitmap);
		kmem_cache_free(apfs_free_space_cachep, info);
	}
 ...
 *
 * The above rules are for when we merge free space based on apfs_trim_state.
 * Rules 2 and 3 are subtle because they are suboptimal, but are done for the
 * same reason: to promote larger extent regions which makes life easier for
 * find_free_extent().  Rule 2 enables coalescing based on the common path
 * being returning free space from apfs_finish_extent_commit().  So when free
 * space is trimmed, it will prevent aggregating trimmed new region and
 * untrimmed regions in the rb_tree.  Rule 3 is purely to obtain larger extents
 * and provide find_free_extent() with the largest extents possible hoping for
 * the reuse path.
 */
static bool try_merge_free_space(struct apfs_free_space_ctl *ctl,
			  struct apfs_free_space *info, bool update_stat)
{
	struct apfs_free_space *left_info = NULL;
	struct apfs_free_space *right_info;
	bool merged = false;
	u64 offset = info->offset;
	u64 bytes = info->bytes;
	const bool is_trimmed = apfs_free_space_trimmed(info);
	...
	if (right_info && rb_prev(&right_info->offset_index))
		left_info = rb_entry(rb_prev(&right_info->offset_index),
				     struct apfs_free_space, offset_index);
	else if (!right_info)
		left_info = tree_search_offset(ctl, offset - 1, 0, 0);

	/* See try_merge_free_space() comment. */
	if (right_info && !right_info->bitmap &&
	    (!is_trimmed || apfs_free_space_trimmed(right_info))) {
		if (update_stat)
			unlink_free_space(ctl, right_info);
		else
			__unlink_free_space(ctl, right_info);
		info->bytes += right_info->bytes;
		kmem_cache_free(apfs_free_space_cachep, right_info);
		merged = true;
	}
	...
	if (left_info && !left_info->bitmap &&
	    ...
	    (!is_trimmed || apfs_free_space_trimmed(left_info))) {
		if (update_stat)
		...
		kmem_cache_free(apfs_free_space_cachep, left_info);
		merged = true;
	}
...
static bool steal_from_bitmap_to_end(struct apfs_free_space_ctl *ctl,
				     struct apfs_free_space *info,
				     bool update_stat)
{
	struct apfs_free_space *bitmap;
	unsigned long i;
	unsigned long j;
	...
	/* See try_merge_free_space() comment. */
	if (!apfs_free_space_trimmed(bitmap))
		info->trim_state = APFS_TRIM_STATE_UNTRIMMED;
...
static bool steal_from_bitmap_to_front(struct apfs_free_space_ctl *ctl,
				       struct apfs_free_space *info,
				       bool update_stat)
{
	struct apfs_free_space *bitmap;
 ...
 * ends).
 */
static void steal_from_bitmap(struct apfs_free_space_ctl *ctl,
			      struct apfs_free_space *info,
			      bool update_stat)
{
	/*
			   ...
			   enum apfs_trim_state trim_state)
{
	...
	ASSERT(!apfs_is_zoned(fs_info));

	info = kmem_cache_zalloc(apfs_free_space_cachep, GFP_NOFS);
	if (!info)
		return -ENOMEM;
...
out:
	...
	if (ret) {
		apfs_crit(fs_info, "unable to add free space :%d", ret);
		ASSERT(ret != -EEXIST);
	}

	if (trim_state != APFS_TRIM_STATE_TRIMMED) {
		apfs_discard_check_filter(block_group, filter_bytes);
		apfs_discard_queue_work(&fs_info->discard_ctl, block_group);
	}

	return ret;
}

static int __apfs_add_free_space_zoned(struct apfs_block_group *block_group,
...
int apfs_add_free_space(struct apfs_block_group *block_group,
			 u64 bytenr, u64 size)
{
	enum apfs_trim_state trim_state = APFS_TRIM_STATE_UNTRIMMED;

	if (apfs_is_zoned(block_group->fs_info))
		return __apfs_add_free_space_zoned(block_group, bytenr, size,
						    true);

	if (apfs_test_opt(block_group->fs_info, DISCARD_SYNC))
		trim_state = APFS_TRIM_STATE_TRIMMED;

	return __apfs_add_free_space(block_group->fs_info,
				...
				u64 bytenr, u64 size)
{
	...
	return apfs_add_free_space(block_group, bytenr, size);
}
				       ...
				       u64 bytenr, u64 size)
{
	...
	if (apfs_test_opt(block_group->fs_info, DISCARD_SYNC) ||
	    apfs_test_opt(block_group->fs_info, DISCARD_ASYNC))
		trim_state = APFS_TRIM_STATE_TRIMMED;
			    ...
			    u64 offset, u64 bytes)
{
	...
	if (apfs_is_zoned(block_group->fs_info)) {
		 ...
		 * apfs_pin_extent_for_log_replay() when replaying the log.
...
again:
	...
	if (!info->bitmap) {
		...
		if (offset == info->offset) {
			...
			ret = __apfs_add_free_space(block_group->fs_info, ctl,
						     offset + bytes,
			   ...
			   u64 bytes)
{
	...
	/*
	 * Zoned apfs does not use free space tree and cluster. Just print
	 * out the free space after the allocation offset.
	 */
	if (apfs_is_zoned(fs_info)) {
		apfs_info(fs_info, "free space %llu",
	...
	for (n = rb_first(&ctl->free_space_offset); n; n = rb_next(n)) {
		...
		if (info->bytes >= bytes && !block_group->ro)
			count++;
		apfs_crit(fs_info, "entry offset %llu, bytes %llu, bitmap %s",
			   info->offset, info->bytes,
		       (info->bitmap) ? "yes" : "no");
	}
	spin_unlock(&ctl->tree_lock);
	apfs_info(fs_info, "block group has cluster?: %s",
			       ...
			       struct apfs_free_space_ctl *ctl)
{
	 ...
	 */
	ctl->extents_thresh = (SZ_32K / 2) / sizeof(struct apfs_free_space);
}
 ...
 */
static void __apfs_return_cluster_to_free_space(
			     struct apfs_block_group *block_group,
			     struct apfs_free_cluster *cluster)
{
	struct apfs_free_space_ctl *ctl = block_group->free_space_ctl;
	...
	while (node) {
		...
		if (!bitmap) {
			...
			if (!apfs_free_space_trimmed(entry)) {
				ctl->discardable_extents[APFS_STAT_CURR]--;
				ctl->discardable_bytes[APFS_STAT_CURR] -=
					entry->bytes;
			}
...
static void __apfs_remove_free_space_cache_locked(
				struct apfs_free_space_ctl *ctl)
{
	struct apfs_free_space *info;
	struct rb_node *node;
	...
	while ((node = rb_last(&ctl->free_space_offset)) != NULL) {
		info = rb_entry(node, struct apfs_free_space, offset_index);
		if (!info->bitmap) {
			unlink_free_space(ctl, info);
			kmem_cache_free(apfs_free_space_cachep, info);
		} else {
...
{
	spin_lock(&ctl->tree_lock);
	__apfs_remove_free_space_cache_locked(ctl);
	if (ctl->private)
		apfs_discard_update_discardable(ctl->private);
	spin_unlock(&ctl->tree_lock);
}
...
void apfs_remove_free_space_cache(struct apfs_block_group *block_group)
{
	...
	while ((head = block_group->cluster_list.next) !=
		...
		cluster = list_entry(head, struct apfs_free_cluster,
				     block_group_list);

		WARN_ON(cluster->block_group != block_group);
		__apfs_return_cluster_to_free_space(block_group, cluster);
...
/**
 * apfs_is_free_space_trimmed - see if everything is trimmed
			       ...
			       u64 *max_extent_size)
{
	...
	struct apfs_free_space *entry = NULL;
	u64 bytes_search = bytes + empty_size;
	u64 ret = 0;
	u64 align_gap = 0;
	u64 align_gap_len = 0;
	enum apfs_trim_state align_gap_trim_state = APFS_TRIM_STATE_UNTRIMMED;
	...
	if (entry->bitmap) {
		...
		if (!entry->bytes)
			kmem_cache_free(apfs_free_space_cachep, entry);
		else
...
out:
	...
	if (align_gap_len)
		__apfs_add_free_space(block_group->fs_info, ctl,
			       ...
			       struct apfs_free_cluster *cluster)
{
	...
	apfs_discard_queue_work(&block_group->fs_info->discard_ctl, block_group);

	/* finally drop our ref */
	apfs_put_block_group(block_group);
}

static u64 apfs_alloc_from_bitmap(struct apfs_block_group *block_group,
				   struct apfs_free_cluster *cluster,
				   struct apfs_free_space *entry,
			     ...
			     u64 min_start, u64 *max_extent_size)
{
	...
	while (1) {
		...
		if (entry->bytes < bytes ||
			...
			if (!node)
				break;
			entry = rb_entry(node, struct apfs_free_space,
					 offset_index);
			continue;
		}

		if (entry->bitmap) {
			ret = apfs_alloc_from_bitmap(block_group,
						      cluster, entry, bytes,
...
out:
	...
	ctl->free_space -= bytes;
	if (!entry->bitmap && !apfs_free_space_trimmed(entry))
		ctl->discardable_bytes[APFS_STAT_CURR] -= bytes;
	...
	if (entry->bytes == 0) {
		...
		if (entry->bitmap) {
			...
			ctl->total_bitmaps--;
			recalculate_thresholds(ctl);
		} else if (!apfs_free_space_trimmed(entry)) {
			ctl->discardable_extents[APFS_STAT_CURR]--;
		}
		kmem_cache_free(apfs_free_space_cachep, entry);
	}
...
static int apfs_bitmap_cluster(struct apfs_block_group *block_group,
				struct apfs_free_space *entry,
				struct apfs_free_cluster *cluster,
				u64 offset, u64 bytes,
...
again:
	...
	trace_apfs_setup_cluster(block_group, cluster,
 ...
 */
static noinline int
setup_cluster_no_bitmap(struct apfs_block_group *block_group,
			struct apfs_free_cluster *cluster,
			struct list_head *bitmaps, u64 offset, u64 bytes,
			u64 cont1_bytes, u64 min_bytes)
{
	struct apfs_free_space_ctl *ctl = block_group->free_space_ctl;
	struct apfs_free_space *first = NULL;
	struct apfs_free_space *entry = NULL;
	struct apfs_free_space *last;
	struct rb_node *node;
	u64 window_free;
	u64 max_extent;
	u64 total_size = 0;
	...
	trace_apfs_setup_cluster(block_group, cluster, total_size, 0);
	return 0;
}
 ...
 */
static noinline int
setup_cluster_bitmap(struct apfs_block_group *block_group,
		     struct apfs_free_cluster *cluster,
		     struct list_head *bitmaps, u64 offset, u64 bytes,
		     u64 cont1_bytes, u64 min_bytes)
{
	...
	if (!list_empty(bitmaps))
		entry = list_first_entry(bitmaps, struct apfs_free_space, list);
	...
	list_for_each_entry(entry, bitmaps, list) {
		if (entry->bytes < bytes)
			continue;
		ret = apfs_bitmap_cluster(block_group, entry, cluster, offset,
			     ...
			     u64 offset, u64 bytes, u64 empty_size)
{
	struct apfs_fs_info *fs_info = block_group->fs_info;
	struct apfs_free_space_ctl *ctl = block_group->free_space_ctl;
	struct apfs_free_space *entry, *tmp;
	LIST_HEAD(bitmaps);
	u64 min_bytes;
	u64 cont1_bytes;
	int ret;
	 ...
	 * data, keep it dense.
	 */
	if (apfs_test_opt(fs_info, SSD_SPREAD)) {
		cont1_bytes = min_bytes = bytes + empty_size;
	} else if (block_group->flags & APFS_BLOCK_GROUP_METADATA) {
		cont1_bytes = bytes;
	...
	trace_apfs_find_cluster(block_group, offset, bytes, empty_size,
				 min_bytes);
	...
	if (!ret) {
		...
		trace_apfs_failed_cluster_setup(block_group);
	}
out:
	spin_unlock(&cluster->lock);
	spin_unlock(&ctl->tree_lock);
...
static int do_trimming(struct apfs_block_group *block_group,
		       u64 *total_trimmed, u64 start, u64 bytes,
		       u64 reserved_start, u64 reserved_bytes,
		       enum apfs_trim_state reserved_trim_state,
		       struct apfs_trim_range *trim_entry)
{
	...
	ret = apfs_discard_extent(fs_info, start, bytes, &trimmed);
	if (!ret) {
	...
	if (reserved_start < start)
		__apfs_add_free_space(fs_info, ctl, reserved_start,
				       start - reserved_start,
				       reserved_trim_state);
	if (start + bytes < reserved_start + reserved_bytes)
		__apfs_add_free_space(fs_info, ctl, end, reserved_end - end,
				       reserved_trim_state);
	__apfs_add_free_space(fs_info, ctl, start, bytes, trim_state);
 ...
 */
static int trim_no_bitmap(struct apfs_block_group *block_group,
			  ...
			  bool async)
{
	...
	u64 extent_start;
	u64 extent_bytes;
	enum apfs_trim_state extent_trim_state;
	u64 bytes;
	...
	while (start < end) {
		...
		while (entry->bitmap ||
		       (async && apfs_free_space_trimmed(entry))) {
			node = rb_next(&entry->offset_index);
			if (!node)
				goto out_unlock;
			entry = rb_entry(node, struct apfs_free_space,
		...
		if (async) {
			...
			unlink_free_space(ctl, entry);
			/*
			 * Let bytes = APFS_MAX_DISCARD_SIZE + X.
			 * If X < APFS_ASYNC_DISCARD_MIN_FILTER, we won't trim
			 * X when we come back around.  So trim it now.
			 */
			if (max_discard_size &&
			    bytes >= (max_discard_size +
				      APFS_ASYNC_DISCARD_MIN_FILTER)) {
				bytes = max_discard_size;
...
out_unlock:
	block_group->discard_cursor = apfs_block_group_end(block_group);
	spin_unlock(&ctl->tree_lock);
	mutex_unlock(&ctl->cache_writeout_mutex);
 ...
 * reset the state to APFS_TRIM_STATE_UNTRIMMED.
 *
 ...
 */
static void reset_trimming_bitmap(struct apfs_free_space_ctl *ctl, u64 offset)
{
	struct apfs_free_space *entry;
	...
	if (entry) {
		if (apfs_free_space_trimmed(entry)) {
			...
			ctl->discardable_bytes[APFS_STAT_CURR] += entry->bytes;
		}
		entry->trim_state = APFS_TRIM_STATE_UNTRIMMED;
	}

	spin_unlock(&ctl->tree_lock);
}

static void end_trimming_bitmap(struct apfs_free_space_ctl *ctl,
				struct apfs_free_space *entry)
{
	if (apfs_free_space_trimming_bitmap(entry)) {
		entry->trim_state = APFS_TRIM_STATE_TRIMMED;
		ctl->discardable_extents[APFS_STAT_CURR] -=
			entry->bitmap_extents;
		ctl->discardable_bytes[APFS_STAT_CURR] -= entry->bytes;
	}
 ...
 * If @async is set, then we will trim 1 region and return.
 */
static int trim_bitmaps(struct apfs_block_group *block_group,
			u64 *total_trimmed, u64 start, u64 end, u64 minlen,
	...
	while (offset < end) {
		...
		if (ctl->free_space < minlen) {
			block_group->discard_cursor =
				apfs_block_group_end(block_group);
			spin_unlock(&ctl->tree_lock);
			mutex_unlock(&ctl->cache_writeout_mutex);
			break;
		}
		 ...
		 * @minlen to determine if we are in APFS_DISCARD_INDEX_UNUSED
		 * which is the only discard index which sets minlen to 0.
		 */
		if (!entry || (async && minlen && start == offset &&
			       apfs_free_space_trimmed(entry))) {
			spin_unlock(&ctl->tree_lock);
			mutex_unlock(&ctl->cache_writeout_mutex);
			next_bitmap = true;
			goto next;
		 ...
		 */
		if (start == offset)
			entry->trim_state = APFS_TRIM_STATE_TRIMMING;
		...
		if (ret2 || start >= end) {
			 ...
			 * over regions <= APFS_ASYNC_DISCARD_MIN_FILTER.
			 */
			if (ret2 && minlen <= APFS_ASYNC_DISCARD_MIN_FILTER)
...
{
	return apfs_super_cache_generation(fs_info->super_copy);
}

static int cleanup_free_space_cache_v1(struct apfs_fs_info *fs_info,
				       struct apfs_trans_handle *trans)
{
	struct apfs_block_group *block_group;
	struct rb_node *node;
	...
	apfs_info(fs_info, "cleaning free space cache v1");

	node = rb_first(&fs_info->block_group_cache_tree);
	while (node) {
		block_group = rb_entry(node, struct apfs_block_group, cache_node);
		ret = apfs_remove_free_space_inode(trans, NULL, block_group);
		if (ret)
			goto out;
		node = rb_next(node);
	}
...
int apfs_set_free_space_cache_v1_active(struct apfs_fs_info *fs_info, bool active)
{
	 ...
	 * APFS_FS_CLEANUP_SPACE_CACHE_V1. For this reason, we need a
	...
	if (!active) {
		set_bit(APFS_FS_CLEANUP_SPACE_CACHE_V1, &fs_info->flags);
...
out:
	clear_bit(APFS_FS_CLEANUP_SPACE_CACHE_V1, &fs_info->flags);
...
{
	struct apfs_free_space_ctl *ctl = cache->free_space_ctl;
	struct apfs_free_space *info = NULL, *bitmap_info;
	void *map = NULL;
	enum apfs_trim_state trim_state = APFS_TRIM_STATE_TRIMMED;
	u64 bytes_added;
	int ret;
...
again:
	...
	if (!map) {
		map = kmem_cache_zalloc(apfs_free_space_bitmap_cachep, GFP_NOFS);
		if (!map) {
			kmem_cache_free(apfs_free_space_cachep, info);
			return -ENOMEM;
		}
	...
	if (info)
		kmem_cache_free(apfs_free_space_cachep, info);
	if (map)
		kmem_cache_free(apfs_free_space_bitmap_cachep, map);
	return 0;
}
 ...
 */
int test_check_exists(struct apfs_block_group *cache,
		      u64 offset, u64 bytes)
{
	struct apfs_free_space_ctl *ctl = cache->free_space_ctl;
	struct apfs_free_space *info;
...
have_info:
	if (info->bitmap) {
		...
		while (n) {
			tmp = rb_entry(n, struct apfs_free_space,
				       offset_index);
...
#endif /* CONFIG_APFS_FS_RUN_SANITY_TESTS */
