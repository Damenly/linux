...
#define APFS_DISCARD_DELAY		(120ULL * NSEC_PER_SEC)
#define APFS_DISCARD_UNUSED_DELAY	(10ULL * NSEC_PER_SEC)

/* Target completion latency of discarding all discardable extents */
#define APFS_DISCARD_TARGET_MSEC	(6 * 60 * 60UL * MSEC_PER_SEC)
#define APFS_DISCARD_MIN_DELAY_MSEC	(1UL)
#define APFS_DISCARD_MAX_DELAY_MSEC	(1000UL)
#define APFS_DISCARD_MAX_IOPS		(10U)

/* Montonically decreasing minimum length filters after index 0 */
static int discard_minlen[APFS_NR_DISCARD_LISTS] = {
	0,
	APFS_ASYNC_DISCARD_MAX_FILTER,
	APFS_ASYNC_DISCARD_MIN_FILTER
};

static struct list_head *get_discard_list(struct apfs_discard_ctl *discard_ctl,
					  struct apfs_block_group *block_group)
{
	return &discard_ctl->discard_list[block_group->discard_index];
}

static void __add_to_discard_list(struct apfs_discard_ctl *discard_ctl,
				  struct apfs_block_group *block_group)
{
	if (!apfs_run_discard_work(discard_ctl))
		return;

	if (list_empty(&block_group->discard_list) ||
	    block_group->discard_index == APFS_DISCARD_INDEX_UNUSED) {
		if (block_group->discard_index == APFS_DISCARD_INDEX_UNUSED)
			block_group->discard_index = APFS_DISCARD_INDEX_START;
		block_group->discard_eligible_time = (ktime_get_ns() +
						      APFS_DISCARD_DELAY);
		block_group->discard_state = APFS_DISCARD_RESET_CURSOR;
	}
...
static void add_to_discard_list(struct apfs_discard_ctl *discard_ctl,
				struct apfs_block_group *block_group)
{
	if (!apfs_is_block_group_data_only(block_group))
		return;
...
static void add_to_discard_unused_list(struct apfs_discard_ctl *discard_ctl,
				       struct apfs_block_group *block_group)
{
	spin_lock(&discard_ctl->lock);
	...
	block_group->discard_index = APFS_DISCARD_INDEX_UNUSED;
	block_group->discard_eligible_time = (ktime_get_ns() +
					      APFS_DISCARD_UNUSED_DELAY);
	block_group->discard_state = APFS_DISCARD_RESET_CURSOR;
	list_add_tail(&block_group->discard_list,
		      &discard_ctl->discard_list[APFS_DISCARD_INDEX_UNUSED]);

	spin_unlock(&discard_ctl->lock);
}

static bool remove_from_discard_list(struct apfs_discard_ctl *discard_ctl,
				     struct apfs_block_group *block_group)
{
	bool running = false;
 ...
 */
static struct apfs_block_group *find_next_block_group(
					struct apfs_discard_ctl *discard_ctl,
					u64 now)
{
	struct apfs_block_group *ret_block_group = NULL, *block_group;
	int i;

	for (i = 0; i < APFS_NR_DISCARD_LISTS; i++) {
 ...
 */
static struct apfs_block_group *peek_discard_list(
					struct apfs_discard_ctl *discard_ctl,
...
again:
	...
	if (block_group && now >= block_group->discard_eligible_time) {
		...
		if (block_group->discard_state == APFS_DISCARD_RESET_CURSOR) {
			block_group->discard_cursor = block_group->start;
			block_group->discard_state = APFS_DISCARD_EXTENTS;
		}
...
/**
 * apfs_discard_check_filter - updates a block groups filters
 ...
 */
void apfs_discard_check_filter(struct apfs_block_group *block_group,
				u64 bytes)
{
	struct apfs_discard_ctl *discard_ctl;
	...
	if (block_group->discard_index > APFS_DISCARD_INDEX_START &&
	    bytes >= discard_minlen[block_group->discard_index - 1]) {
		int i;

		remove_from_discard_list(discard_ctl, block_group);

		for (i = APFS_DISCARD_INDEX_START; i < APFS_NR_DISCARD_LISTS;
		     i++) {
...
/**
 * apfs_update_discard_index - moves a block group along the discard lists
 * @discard_ctl: discard control
 ...
 */
static void apfs_update_discard_index(struct apfs_discard_ctl *discard_ctl,
				       struct apfs_block_group *block_group)
{
	block_group->discard_index++;
	if (block_group->discard_index == APFS_NR_DISCARD_LISTS) {
...
/**
 * apfs_discard_cancel_work - remove a block_group from the discard lists
 * @discard_ctl: discard control
 * @block_group: block_group of interest
 *
...
/**
 * apfs_discard_queue_work - handles queuing the block_groups
 * @discard_ctl: discard control
 * @block_group: block_group of interest
 *
...
void apfs_discard_queue_work(struct apfs_discard_ctl *discard_ctl,
			      struct apfs_block_group *block_group)
{
	if (!block_group || !apfs_test_opt(block_group->fs_info, DISCARD_ASYNC))
		return;
	...
	if (!delayed_work_pending(&discard_ctl->work))
		apfs_discard_schedule_work(discard_ctl, false);
}

static void __apfs_discard_schedule_work(struct apfs_discard_ctl *discard_ctl,
...
/*
 * apfs_discard_schedule_work - responsible for scheduling the discard work
 * @discard_ctl:  discard control
				 ...
				 bool override)
{
	...
	spin_lock(&discard_ctl->lock);
	__apfs_discard_schedule_work(discard_ctl, now, override);
	spin_unlock(&discard_ctl->lock);
}

/**
 * apfs_finish_discard_pass - determine next step of a block_group
 * @discard_ctl: discard control
 * @block_group: block_group of interest
 *
 ...
 */
static void apfs_finish_discard_pass(struct apfs_discard_ctl *discard_ctl,
				      struct apfs_block_group *block_group)
{
	remove_from_discard_list(discard_ctl, block_group);

	if (block_group->used == 0) {
		if (apfs_is_free_space_trimmed(block_group))
			apfs_mark_bg_unused(block_group);
		else
			add_to_discard_unused_list(discard_ctl, block_group);
	} else {
		apfs_update_discard_index(discard_ctl, block_group);
	}
}

/**
 * apfs_discard_workfn - discard work function
 * @work: work
 *
 ...
 */
static void apfs_discard_workfn(struct work_struct *work)
{
	struct apfs_discard_ctl *discard_ctl;
	struct apfs_block_group *block_group;
	enum apfs_discard_state discard_state;
	...
	discard_ctl = container_of(work, struct apfs_discard_ctl, work.work);

	block_group = peek_discard_list(discard_ctl, &discard_state,
					&discard_index, now);
	if (!block_group || !apfs_run_discard_work(discard_ctl))
		return;
	if (now < block_group->discard_eligible_time) {
		apfs_discard_schedule_work(discard_ctl, false);
		return;
	...
	if (discard_state == APFS_DISCARD_BITMAPS) {
		u64 maxlen = 0;
		 ...
		 * back to APFS_TRIM_STATE_UNTRIMMED.
		 */
		if (discard_index != APFS_DISCARD_INDEX_UNUSED)
			maxlen = discard_minlen[discard_index - 1];

		apfs_trim_block_group_bitmaps(block_group, &trimmed,
				       block_group->discard_cursor,
				       apfs_block_group_end(block_group),
				       minlen, maxlen, true);
		discard_ctl->discard_bitmap_bytes += trimmed;
	} else {
		apfs_trim_block_group_extents(block_group, &trimmed,
				       block_group->discard_cursor,
				       apfs_block_group_end(block_group),
				       minlen, true);
	...
	if (block_group->discard_cursor >= apfs_block_group_end(block_group)) {
		if (discard_state == APFS_DISCARD_BITMAPS) {
			apfs_finish_discard_pass(discard_ctl, block_group);
		} else {
			block_group->discard_cursor = block_group->start;
			spin_lock(&discard_ctl->lock);
	...
	discard_ctl->block_group = NULL;
	__apfs_discard_schedule_work(discard_ctl, now, false);
	spin_unlock(&discard_ctl->lock);
}

/**
 * apfs_run_discard_work - determines if async discard should be running
 * @discard_ctl: discard control
 *
 * Checks if the file system is writeable and APFS_FS_DISCARD_RUNNING is set.
 */
bool apfs_run_discard_work(struct apfs_discard_ctl *discard_ctl)
{
	struct apfs_fs_info *fs_info = container_of(discard_ctl,
						     struct apfs_fs_info,
						     discard_ctl);

	return (!(fs_info->sb->s_flags & SB_RDONLY) &&
		test_bit(APFS_FS_DISCARD_RUNNING, &fs_info->flags));
}

/**
 * apfs_discard_calc_delay - recalculate the base delay
 * @discard_ctl: discard control
 *
 * Recalculate the base delay which is based off the total number of
 * discardable_extents.  Clamp this between the lower_limit (iops_limit or 1ms)
 * and the upper_limit (APFS_DISCARD_MAX_DELAY_MSEC).
 */
void apfs_discard_calc_delay(struct apfs_discard_ctl *discard_ctl)
{
	 ...
	 * apfs_finish_extent_commit() which is synchronized, we can correct
	 * here.
	 */
	...
	if (iops_limit)
		...
		delay = APFS_DISCARD_TARGET_MSEC / discardable_extents;

	delay = clamp(delay, APFS_DISCARD_MIN_DELAY_MSEC,
		      APFS_DISCARD_MAX_DELAY_MSEC);
...
/**
 * apfs_discard_update_discardable - propagate discard counters
 * @block_group: block_group of interest
 *
...
void apfs_discard_update_discardable(struct apfs_block_group *block_group)
{
	...
	if (!block_group ||
	    !apfs_test_opt(block_group->fs_info, DISCARD_ASYNC) ||
	    !apfs_is_block_group_data_only(block_group))
		return;
	...
	extents_delta = ctl->discardable_extents[APFS_STAT_CURR] -
			ctl->discardable_extents[APFS_STAT_PREV];
	if (extents_delta) {
	...
	bytes_delta = ctl->discardable_bytes[APFS_STAT_CURR] -
		      ctl->discardable_bytes[APFS_STAT_PREV];
	if (bytes_delta) {
...
/**
 * apfs_discard_punt_unused_bgs_list - punt unused_bgs list to discard lists
...
{
	struct apfs_block_group *block_group, *next;
...
/**
 * apfs_discard_purge_list - purge discard lists
 * @discard_ctl: discard control
 *
 ...
 */
static void apfs_discard_purge_list(struct apfs_discard_ctl *discard_ctl)
{
	struct apfs_block_group *block_group, *next;
	int i;
...
void apfs_discard_resume(struct apfs_fs_info *fs_info)
{
	...
	set_bit(APFS_FS_DISCARD_RUNNING, &fs_info->flags);
}

void apfs_discard_stop(struct apfs_fs_info *fs_info)
{
	clear_bit(APFS_FS_DISCARD_RUNNING, &fs_info->flags);
}
...
void apfs_discard_init(struct apfs_fs_info *fs_info)
{
	...
	INIT_DELAYED_WORK(&discard_ctl->work, apfs_discard_workfn);
	...
	discard_ctl->max_discard_size = APFS_ASYNC_DISCARD_DEFAULT_MAX_SIZE;
	discard_ctl->delay_ms = APFS_DISCARD_MAX_DELAY_MSEC;
	discard_ctl->iops_limit = APFS_DISCARD_MAX_IOPS;
...
void apfs_discard_cleanup(struct apfs_fs_info *fs_info)
{
	...
	apfs_discard_purge_list(&fs_info->discard_ctl);
}
