			   ...
			   struct share_check *sc)
{
	...
	while (ptr < end) {
		...
		case APFS_SHARED_DATA_REF_KEY: {
			...
			sdref = (struct apfs_shared_data_ref *)(iref + 1);
			count = apfs_shared_data_ref_count(leaf, sdref);
		...
		case APFS_EXTENT_DATA_REF_KEY: {
			...
			count = apfs_extent_data_ref_count(leaf, dref);
			key.objectid = apfs_extent_data_ref_objectid(leaf,
								      dref);
			key.type = APFS_EXTENT_DATA_KEY;
			key.offset = apfs_extent_data_ref_offset(leaf, dref);
			...
			root = apfs_extent_data_ref_root(leaf, dref);
 ...
 * Returns 0 on success, <0 on error, or BACKREF_FOUND_SHARED.
 */
static int add_keyed_refs(struct apfs_fs_info *fs_info,
			  struct apfs_path *path, u64 bytenr,
			  int info_level, struct preftrees *preftrees,
			  struct share_check *sc)
{
	...
	while (1) {
		...
		if (key.objectid != bytenr)
			break;
		if (key.type < APFS_TREE_BLOCK_REF_KEY)
			continue;
 ...
 *
 * If time_seq is set to APFS_SEQ_LAST, it will not search delayed_refs, and
 ...
 */
static int find_parent_nodes(struct apfs_trans_handle *trans,
			     ...
			     struct share_check *sc, bool ignore_offset)
{
	...
	struct apfs_delayed_ref_root *delayed_refs = NULL;
...
again:
	...
	if (trans && likely(trans->type != __TRANS_DUMMY) &&
	    time_seq != APFS_SEQ_LAST) {
#else
	if (trans && time_seq != APFS_SEQ_LAST) {
#endif
		/*
	...
	while (node) {
		 ...
		 * refs with a node->action of APFS_DROP_DELAYED_REF.
 ...
 * returns 0 on success, < 0 on error.
 */
static int apfs_find_all_roots_safe(struct apfs_trans_handle *trans,
				     struct apfs_fs_info *fs_info, u64 bytenr,
			 ...
			 bool ignore_offset, bool skip_commit_root_sem)
{
	...
	ret = apfs_find_all_roots_safe(trans, fs_info, bytenr,
 ...
 *
 * apfs_check_shared uses the backref walking code but will short
		...
		struct ulist *roots, struct ulist *tmp)
{
	...
	struct ulist_iterator uiter;
	struct ulist_node *node;
	struct apfs_seq_list elem = APFS_SEQ_LIST_INIT(elem);
	int ret = 0;
	...
	trans = apfs_join_transaction_nostart(root);
	if (IS_ERR(trans)) {
		...
		apfs_get_tree_mod_seq(fs_info, &elem);
	}
	...
	if (trans) {
		apfs_put_tree_mod_seq(fs_info, &elem);
			  ...
			  u64 *found_off)
{
	...
	while (1) {
		...
		if (slot >= apfs_header_nritems(leaf)) {
			 ...
			 * apfs_search_slot will point us to the slot
			...
			char *dest, u32 size)
{
	...
	while (1) {
		...
		ret = apfs_find_item(fs_root, path, parent, 0,
				APFS_INODE_REF_KEY, &found_key);
		if (ret > 0)
			ret = -ENOENT;
		if (ret)
			break;
		...
		apfs_release_path(path);
		iref = apfs_item_ptr(eb, slot, struct apfs_inode_ref);
 ...
 * returns APFS_EXTENT_FLAG_DATA for data, APFS_EXTENT_FLAG_TREE_BLOCK for
			...
			u64 *flags_ret)
{
	...
	ret = apfs_previous_extent_item(fs_info->extent_root, path, 0);
	if (ret) {
		if (ret > 0)
			ret = -ENOENT;
		return ret;
	...
	if (found_key->type == APFS_METADATA_ITEM_KEY)
		size = fs_info->nodesize;
	else if (found_key->type == APFS_EXTENT_ITEM_KEY)
	...
	if (flags_ret) {
		if (flags & APFS_EXTENT_FLAG_TREE_BLOCK)
			*flags_ret = APFS_EXTENT_FLAG_TREE_BLOCK;
		else if (flags & APFS_EXTENT_FLAG_DATA)
			*flags_ret = APFS_EXTENT_FLAG_DATA;
		else
			BUG();
		return 0;
	}
...
static int get_extent_inline_ref(unsigned long *ptr,
				 ...
				 const struct apfs_extent_item *ei,
				 u32 item_size,
				 struct apfs_extent_inline_ref **out_eiref,
				 int *out_type)
{
	unsigned long end;
	u64 flags;
	...
	if (!*ptr) {
		...
		if (flags & APFS_EXTENT_FLAG_TREE_BLOCK) {
			if (key->type == APFS_METADATA_ITEM_KEY) {
				...
				*out_eiref =
				     (struct apfs_extent_inline_ref *)(ei + 1);
			} else {
				WARN_ON(key->type != APFS_EXTENT_ITEM_KEY);
				info = (struct apfs_tree_block_info *)(ei + 1);
				*out_eiref =
				   (struct apfs_extent_inline_ref *)(info + 1);
			}
		} else {
			*out_eiref = (struct apfs_extent_inline_ref *)(ei + 1);
		}
	...
	*out_eiref = (struct apfs_extent_inline_ref *)(*ptr);
	*out_type = apfs_get_extent_inline_ref_type(eb, *out_eiref,
						     APFS_REF_TYPE_ANY);
	if (*out_type == APFS_REF_TYPE_INVALID)
		return -EUCLEAN;

	*ptr += apfs_extent_inline_ref_size(*out_type);
	WARN_ON(*ptr > end);
	if (*ptr == end)
		return 1; /* last */
			    ...
			    u32 item_size, u64 *out_root, u8 *out_level)
{
	int ret;
	int type;
	struct apfs_extent_inline_ref *eiref;
	...
	*out_root = apfs_extent_inline_ref_offset(eb, eiref);

	if (key->type == APFS_EXTENT_ITEM_KEY) {
		struct apfs_tree_block_info *info;

		info = (struct apfs_tree_block_info *)(ei + 1);
		*out_level = apfs_tree_block_level(eb, info);
	} else {
		ASSERT(key->type == APFS_METADATA_ITEM_KEY);
...
static int iterate_leaf_refs(struct apfs_fs_info *fs_info,
				...
				bool ignore_offset)
{
	...
	struct apfs_seq_list seq_elem = APFS_SEQ_LIST_INIT(seq_elem);
	struct ulist_iterator ref_uiter;
	struct ulist_iterator root_uiter;

	apfs_debug(fs_info, "resolving all inodes for extent %llu",
			extent_item_objectid);

	if (!search_commit_root) {
		trans = apfs_attach_transaction(fs_info->extent_root);
		if (IS_ERR(trans)) {
	...
	if (trans)
		apfs_get_tree_mod_seq(fs_info, &seq_elem);
	else
		down_read(&fs_info->commit_root_sem);

	ret = apfs_find_all_leafs(trans, fs_info, extent_item_objectid,
				   seq_elem.seq, &refs,
	...
	while (!ret && (ref_node = ulist_next(refs, &ref_uiter))) {
		ret = apfs_find_all_roots_safe(trans, fs_info, ref_node->val,
						seq_elem.seq, &roots,
						ignore_offset);
		if (ret)
			break;
...
out:
	if (trans) {
		apfs_put_tree_mod_seq(fs_info, &seq_elem);
		apfs_end_transaction(trans);
	} else {
		up_read(&fs_info->commit_root_sem);
	}
...
static int iterate_inode_refs(u64 inum, struct apfs_root *fs_root,
			      struct apfs_path *path,
			      iterate_irefs_t *iterate, void *ctx)
{
	...
	while (!ret) {
		ret = apfs_find_item(fs_root, path, inum,
				parent ? parent + 1 : 0, APFS_INODE_REF_KEY,
				&found_key);
		...
		slot = path->slots[0];
		eb = apfs_clone_extent_buffer(path->nodes[0]);
		if (!eb) {
			ret = -ENOMEM;
			break;
		}
		...
		for (cur = 0; cur < apfs_item_size(eb, item); cur += len) {
			name_len = apfs_inode_ref_name_len(eb, iref);
			/* path must be released before calling iterate()! */
			apfs_debug(fs_root->fs_info,
			...
			iref = (struct apfs_inode_ref *)((char *)iref + len);
		}
		free_extent_buffer(eb);
	}
...
static int iterate_inode_extrefs(u64 inum, struct apfs_root *fs_root,
				 struct apfs_path *path,
				 iterate_irefs_t *iterate, void *ctx)
{
	int ret;
	...
	while (1) {
		ret = apfs_find_one_extref(fs_root, inum, offset, path, &extref,
					    &offset);
		if (ret < 0)
			break;
		if (ret) {
		...
		while (cur_offset < item_size) {
			...
			cur_offset += apfs_inode_extref_name_len(eb, extref);
...
static int iterate_irefs(u64 inum, struct apfs_root *fs_root,
			 struct apfs_path *path, iterate_irefs_t *iterate,
			 void *ctx)
{
	int ret;
	int found_refs = 0;
			 ...
			 struct extent_buffer *eb, void *ctx)
{
	...
	fspath = apfs_ref_to_path(ipath->fs_root, ipath->apfs_path, name_len,
...
{
	return iterate_irefs(inum, ipath->fs_root, ipath->apfs_path,
			     inode_to_path, ipath);
}

struct apfs_data_container *init_data_container(u32 total_bytes)
{
	struct apfs_data_container *data;
	size_t alloc_bytes;
...
{
	struct apfs_backref_iter *ret;

	ret = kzalloc(sizeof(*ret), gfp_flag);
	if (!ret)
		return NULL;

	ret->path = apfs_alloc_path();
	if (!ret->path) {
		kfree(ret);
		return NULL;
	}
...
{
	struct apfs_fs_info *fs_info = iter->fs_info;
	struct apfs_path *path = iter->path;
	struct apfs_extent_item *ei;
	struct apfs_key key = {};
	int ret;
	...
	if ((key.type != APFS_EXTENT_ITEM_KEY &&
	     key.type != APFS_METADATA_ITEM_KEY) || key.objectid != bytenr) {
		ret = -ENOENT;
		goto release;
	}
	memcpy(&iter->cur_key, &key, sizeof(key));
	iter->item_ptr = (u32)apfs_item_ptr_offset(path->nodes[0],
						    path->slots[0]);
	iter->end_ptr = (u32)(iter->item_ptr +
			apfs_item_size_nr(path->nodes[0], path->slots[0]));
	 ...
	 */
	if (apfs_extent_flags(path->nodes[0], ei) & APFS_EXTENT_FLAG_DATA) {
		ret = -ENOTSUPP;
		goto release;
	}
	...
	if (iter->cur_ptr >= iter->end_ptr) {
		ret = apfs_next_item(fs_info->extent_root, path);
		...
		apfs_item_key_to_cpu(path->nodes[0], &iter->cur_key,
				path->slots[0]);
		if (iter->cur_key.objectid != bytenr ||
		    (iter->cur_key.type != APFS_SHARED_BLOCK_REF_KEY &&
		     iter->cur_key.type != APFS_TREE_BLOCK_REF_KEY)) {
			ret = -ENOENT;
			goto release;
		}
		iter->cur_ptr = (u32)apfs_item_ptr_offset(path->nodes[0],
							   path->slots[0]);
		iter->item_ptr = iter->cur_ptr;
		iter->end_ptr = (u32)(iter->item_ptr + apfs_item_size_nr(
				      path->nodes[0], path->slots[0]));
	}

	return 0;
release:
	apfs_backref_iter_release(iter);
	return ret;
}
...
{
	struct extent_buffer *eb = apfs_backref_get_eb(iter);
	struct apfs_path *path = iter->path;
	struct apfs_extent_inline_ref *iref;
	int ret;
	u32 size;

	if (apfs_backref_iter_is_inline_ref(iter)) {
		/* We're still inside the inline refs */
		ASSERT(iter->cur_ptr < iter->end_ptr);

		if (apfs_backref_has_tree_block_info(iter)) {
			/* First tree block info */
			size = sizeof(struct apfs_tree_block_info);
		} else {
			/* Use inline ref type to determine the size */
			int type;

			iref = (struct apfs_extent_inline_ref *)
	...
	ret = apfs_next_item(iter->fs_info->extent_root, iter->path);
	if (ret)
		return ret;

	apfs_item_key_to_cpu(path->nodes[0], &iter->cur_key, path->slots[0]);
	...
	iter->end_ptr = iter->item_ptr + (u32)apfs_item_size_nr(path->nodes[0],
						path->slots[0]);
	return 0;
}
		...
		struct apfs_backref_cache *cache, u64 bytenr, int level)
{
	...
	ASSERT(level >= 0 && level < APFS_MAX_LEVEL);
				...
				struct apfs_backref_node *node)
{
	...
	while (!list_empty(&node->upper)) {
		edge = list_entry(node->upper.next, struct apfs_backref_edge,
				  list[LOWER]);
	...
	apfs_backref_drop_node(cache, node);
}
 ...
 *		type is apfs_inline_ref_type, offset is
 *		apfs_inline_ref_offset.
 */
static int handle_direct_tree_backref(struct apfs_backref_cache *cache,
				      struct apfs_key *ref_key,
				      struct apfs_backref_node *cur)
{
	...
	ASSERT(ref_key->type == APFS_SHARED_BLOCK_REF_KEY);
	...
	edge = apfs_backref_alloc_edge(cache);
	if (!edge)
		return -ENOMEM;
	...
	if (!rb_node) {
		...
		upper = apfs_backref_alloc_node(cache, ref_key->offset,
					   cur->level + 1);
		if (!upper) {
			apfs_backref_free_edge(cache, edge);
			return -ENOMEM;
		...
		upper = rb_entry(rb_node, struct apfs_backref_node, rb_node);
		ASSERT(upper->checked);
		INIT_LIST_HEAD(&edge->list[UPPER]);
	}
	apfs_backref_link_edge(edge, cur, upper, LINK_LOWER);
	return 0;
}
 ...
 */
static int handle_indirect_tree_backref(struct apfs_backref_cache *cache,
					struct apfs_path *path,
					struct apfs_key *ref_key,
					struct apfs_key *tree_key,
					struct apfs_backref_node *cur)
{
	...
	root = apfs_get_fs_root(fs_info, ref_key->offset, false);
	if (IS_ERR(root))
		return PTR_ERR(root);
	if (!test_bit(APFS_ROOT_SHAREABLE, &root->state))
		cur->cowonly = 1;

	if (apfs_root_level(&root->root_item) == cur->level) {
		/* Tree root */
		ASSERT(apfs_root_bytenr(&root->root_item) == cur->bytenr);
		/*
		 * For reloc backref cache, we may ignore reloc root.  But for
		 * general purpose backref cache, we can't rely on
		 * apfs_should_ignore_reloc_root() as it may conflict with
		 ...
		 */
		if (apfs_should_ignore_reloc_root(root) && cache->is_reloc) {
			apfs_put_root(root);
			list_add(&cur->list, &cache->useless_node);
		} else {
			cur->root = root;
	...
	path->skip_locking = 1;
	path->lowest_level = level;
	ret = apfs_search_slot(NULL, root, tree_key, path, 0, 0);
	path->lowest_level = 0;
	if (ret < 0) {
		apfs_put_root(root);
		return ret;
	...
	eb = path->nodes[level];
	if (apfs_node_blockptr(eb, path->slots[level]) != cur->bytenr) {
		apfs_err(fs_info,
"couldn't find block (%llu) (level %d) in tree (%llu) with key (%llu %u %llu)",
	...
	for (; level < APFS_MAX_LEVEL; level++) {
		if (!path->nodes[level]) {
			ASSERT(apfs_root_bytenr(&root->root_item) ==
			       lower->bytenr);
			/* Same as previous should_ignore_reloc_root() call */
			if (apfs_should_ignore_reloc_root(root) &&
			    cache->is_reloc) {
				apfs_put_root(root);
		...
		if (!rb_node) {
			upper = apfs_backref_alloc_node(cache, eb->start,
							 lower->level + 1);
			if (!upper) {
				apfs_put_root(root);
				apfs_backref_free_edge(cache, edge);
			...
			upper->owner = apfs_header_owner(eb);
			if (!test_bit(APFS_ROOT_SHAREABLE, &root->state))
				upper->cowonly = 1;
			 ...
			 */
			if (apfs_block_can_be_shared(root, eb))
				upper->checked = 0;
			else
				upper->checked = 1;
			...
			upper = rb_entry(rb_node, struct apfs_backref_node,
					 rb_node);
			ASSERT(upper->checked);
			INIT_LIST_HEAD(&edge->list[UPPER]);
			if (!upper->owner)
		...
		apfs_backref_link_edge(edge, lower, upper, LINK_LOWER);
 ...
 *	 Use apfs_backref_finish_upper_links() to finish such linkage.
 *
				...
				struct apfs_backref_node *cur)
{
	struct apfs_fs_info *fs_info = cache->fs_info;
	struct apfs_backref_edge *edge;
	struct apfs_backref_node *exist;
	int ret;

	ret = apfs_backref_iter_start(iter, cur->bytenr);
	if (ret < 0)
		return ret;
	/*
	 * We skip the first apfs_tree_block_info, as we don't use the key
	 * stored in it, but fetch it from the tree block
	 */
	if (apfs_backref_has_tree_block_info(iter)) {
		ret = apfs_backref_iter_next(iter);
		if (ret < 0)
			goto out;
	...
	if (!list_empty(&cur->upper)) {
		...
		edge = list_entry(cur->upper.next, struct apfs_backref_edge,
				  list[LOWER]);
	...
	for (; ret == 0; ret = apfs_backref_iter_next(iter)) {
		struct extent_buffer *eb;
		struct apfs_key key = {};
		int type;

		cond_resched();
		eb = apfs_backref_get_eb(iter);
		...
		if (apfs_backref_iter_is_inline_ref(iter)) {
			...
			key.type = type;
			key.offset = apfs_extent_inline_ref_offset(eb, iref);
		} else {
		...
		if (key.type == APFS_SHARED_BLOCK_REF_KEY) {
			...
			goto out;
		} else if (key.type != APFS_TREE_BLOCK_REF_KEY) {
			continue;
		}

		/*
		 * key.type == APFS_TREE_BLOCK_REF_KEY, inline ref offset
...
/*
 * Finish the upwards linkage created by apfs_backref_add_tree_node()
 */
				     ...
				     struct apfs_backref_node *start)
{
	...
	if (!start->cowonly) {
		rb_node = rb_simple_insert(&cache->rb_root, start->bytenr,
					   &start->rb_node);
		if (rb_node)
			apfs_backref_panic(cache->fs_info, start->bytenr,
					    -EEXIST);
	...
	while (!list_empty(&pending_edge)) {
		...
		if (!upper->cowonly) {
			rb_node = rb_simple_insert(&cache->rb_root, upper->bytenr,
						   &upper->rb_node);
			if (rb_node) {
				apfs_backref_panic(cache->fs_info,
						upper->bytenr, -EEXIST);
				return -EUCLEAN;
			}
		}
				 ...
				 struct apfs_backref_node *node)
{
	...
	while (!list_empty(&cache->useless_node)) {
		...
		if (lower == node)
			node = NULL;
		apfs_backref_drop_node(cache, lower);
	}
