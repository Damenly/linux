...
{
	struct apfs_root *root = APFS_I(reloc_inode)->root;
	struct apfs_path *path;
	...
	bytenr -= APFS_I(reloc_inode)->index_cnt;
	ret = apfs_lookup_file_extent(NULL, root, path,
			apfs_ino(APFS_I(reloc_inode)), bytenr, 0);
	if (ret < 0)
		goto out;
	if (ret > 0) {
		ret = -ENOENT;
	...
	BUG_ON(apfs_file_extent_offset(leaf, fi) ||
	       apfs_file_extent_compression(leaf, fi) ||
	       apfs_file_extent_encryption(leaf, fi) ||
	       apfs_file_extent_other_encoding(leaf, fi));

	if (num_bytes != apfs_file_extent_disk_num_bytes(leaf, fi)) {
		ret = -EINVAL;
		goto out;
	}

	*new_bytenr = apfs_file_extent_disk_bytenr(leaf, fi);
	ret = 0;
out:
	apfs_free_path(path);
	return ret;
...
int replace_file_extents(struct apfs_trans_handle *trans,
			 struct reloc_control *rc,
			 struct apfs_root *root,
			 struct extent_buffer *leaf)
{
	...
	for (i = 0; i < nritems; i++) {
		...
		if (key.type != APFS_EXTENT_DATA_KEY)
			continue;
		fi = apfs_item_ptr(leaf, i, struct apfs_file_extent_item);
		...
		if (root->root_key.objectid != APFS_TREE_RELOC_OBJECTID) {
			if (first) {
				...
				first = 0;
			} else if (inode && apfs_ino(APFS_I(inode)) < key.objectid) {
				apfs_add_delayed_iput(inode);
				inode = find_next_inode(root, key.objectid);
			}
			if (inode && apfs_ino(APFS_I(inode)) == key.objectid) {
				end = key.offset +
				...
				ret = try_lock_extent(&APFS_I(inode)->io_tree,
						      key.offset, end);
				if (!ret)
					continue;

				apfs_drop_extent_cache(APFS_I(inode),
						key.offset,	end, 1);
				unlock_extent(&APFS_I(inode)->io_tree,
					      key.offset, end);
			}
		}
		...
		apfs_set_file_extent_disk_bytenr(leaf, fi, new_bytenr);
		dirty = 1;

		key.offset -= apfs_file_extent_offset(leaf, fi);
		apfs_init_generic_ref(&ref, APFS_ADD_DELAYED_REF, new_bytenr,
				       num_bytes, parent);
...
{
	struct apfs_disk_key key1;
	struct apfs_disk_key key2;
	apfs_node_key(eb, &key1, slot);
	apfs_node_key(path->nodes[level], &key2, path->slots[level]);
 ...
 */
static noinline_for_stack
int replace_path(struct apfs_trans_handle *trans, struct reloc_control *rc,
		 struct apfs_root *dest, struct apfs_root *src,
		 struct apfs_path *path, struct apfs_key *next_key,
		 int lowest_level, int max_level)
{
	struct apfs_fs_info *fs_info = dest->fs_info;
	struct extent_buffer *eb;
	...
	ASSERT(src->root_key.objectid == APFS_TREE_RELOC_OBJECTID);
	ASSERT(dest->root_key.objectid != APFS_TREE_RELOC_OBJECTID);

	last_snapshot = apfs_root_last_snapshot(&src->root_item);
again:
	slot = path->slots[lowest_level];
	apfs_node_key_to_cpu(path->nodes[lowest_level], &key, slot);

	eb = apfs_lock_root_node(dest);
	...
	if (cow) {
		ret = apfs_cow_block(trans, dest, eb, NULL, 0, &eb,
				      APFS_NESTING_COW);
		if (ret) {
			apfs_tree_unlock(eb);
			free_extent_buffer(eb);
	...
	parent = eb;
	while (1) {
		level = apfs_header_level(parent);
		ASSERT(level >= lowest_level);

		ret = apfs_bin_search(parent, &key, &slot);
		if (ret < 0)
			break;
		if (ret && slot > 0)
			slot--;

		if (next_key && slot + 1 < apfs_header_nritems(parent))
			apfs_node_key_to_cpu(parent, next_key, slot + 1);

		old_bytenr = apfs_node_blockptr(parent, slot);
		blocksize = fs_info->nodesize;
		old_ptr_gen = apfs_node_ptr_generation(parent, slot);

		if (level <= max_level) {
			eb = path->nodes[level];
			new_bytenr = apfs_node_blockptr(eb,
							path->slots[level]);
			new_ptr_gen = apfs_node_ptr_generation(eb,
							path->slots[level]);
		} else {
			new_bytenr = 0;
			new_ptr_gen = 0;
		...
		if (new_bytenr == 0 || old_ptr_gen > last_snapshot ||
			...
			apfs_tree_lock(eb);
			if (cow) {
				ret = apfs_cow_block(trans, dest, eb, parent,
						      slot, &eb,
						      APFS_NESTING_COW);
				if (ret) {
					apfs_tree_unlock(eb);
		...
		ret = apfs_search_slot(trans, src, &key, path, 0, 1);
		path->lowest_level = 0;
		if (ret) {
			if (ret > 0)
				ret = -ENOENT;
		 ...
		 */
		ret = apfs_qgroup_add_swapped_blocks(trans, dest,
		 ...
		 */
		apfs_set_node_blockptr(parent, slot, new_bytenr);
		apfs_set_node_ptr_generation(parent, slot, new_ptr_gen);
		apfs_mark_buffer_dirty(parent);

		apfs_set_node_blockptr(path->nodes[level],
					path->slots[level], old_bytenr);
		apfs_set_node_ptr_generation(path->nodes[level],
					      path->slots[level], old_ptr_gen);
		apfs_mark_buffer_dirty(path->nodes[level]);

		apfs_init_generic_ref(&ref, APFS_ADD_DELAYED_REF, old_bytenr,
				       blocksize, path->nodes[level]->start);
		ref.skip_qgroup = true;
		apfs_init_tree_ref(&ref, level - 1, src->root_key.objectid);
		ret = apfs_inc_extent_ref(trans, &ref);
		if (ret) {
			apfs_abort_transaction(trans, ret);
			break;
		...
		apfs_init_generic_ref(&ref, APFS_ADD_DELAYED_REF, new_bytenr,
				       blocksize, 0);
		ref.skip_qgroup = true;
		apfs_init_tree_ref(&ref, level - 1, dest->root_key.objectid);
		ret = apfs_inc_extent_ref(trans, &ref);
		if (ret) {
			apfs_abort_transaction(trans, ret);
			break;
		...
		apfs_init_generic_ref(&ref, APFS_DROP_DELAYED_REF, new_bytenr,
				       blocksize, path->nodes[level]->start);
		apfs_init_tree_ref(&ref, level - 1, src->root_key.objectid);
		ref.skip_qgroup = true;
		ret = apfs_free_extent(trans, &ref);
		...
		apfs_init_generic_ref(&ref, APFS_DROP_DELAYED_REF, old_bytenr,
				       blocksize, 0);
		apfs_init_tree_ref(&ref, level - 1, dest->root_key.objectid);
		ref.skip_qgroup = true;
		ret = apfs_free_extent(trans, &ref);
 ...
 */
static noinline_for_stack
int walk_up_reloc_tree(struct apfs_root *root, struct apfs_path *path,
		       int *level)
{
	struct extent_buffer *eb;
	int i;
	...
	last_snapshot = apfs_root_last_snapshot(&root->root_item);
	...
	for (i = *level; i < APFS_MAX_LEVEL && path->nodes[i]; i++) {
		eb = path->nodes[i];
		nritems = apfs_header_nritems(eb);
		while (path->slots[i] + 1 < nritems) {
			path->slots[i]++;
			if (apfs_node_ptr_generation(eb, path->slots[i]) <=
			    last_snapshot)
				continue;
 ...
 */
static noinline_for_stack
int walk_down_reloc_tree(struct apfs_root *root, struct apfs_path *path,
			 int *level)
{
	...
	for (i = *level; i > 0; i--) {
		...
		while (path->slots[i] < nritems) {
			ptr_gen = apfs_node_ptr_generation(eb, path->slots[i]);
		...
		eb = apfs_read_node_slot(eb, path->slots[i]);
		if (IS_ERR(eb))
			return PTR_ERR(eb);
		BUG_ON(apfs_header_level(eb) != i - 1);
		path->nodes[i - 1] = eb;
		path->slots[i - 1] = 0;
	}
	return 1;
 ...
 * [min_key, max_key)
 */
static int invalidate_extent_cache(struct apfs_root *root,
				   ...
				   struct apfs_key *max_key)
{
	...
	while (1) {
		...
		if (unlikely(max_key->objectid == ino)) {
			if (max_key->type < APFS_EXTENT_DATA_KEY)
				continue;
			if (max_key->type > APFS_EXTENT_DATA_KEY) {
				end = (u64)-1;
			} else {
		...
		lock_extent(&APFS_I(inode)->io_tree, start, end);
		apfs_drop_extent_cache(APFS_I(inode), start, end, 1);
		unlock_extent(&APFS_I(inode)->io_tree, start, end);
	}
	return 0;
}
 ...
 */
static int insert_dirty_subvol(struct apfs_trans_handle *trans,
			       struct reloc_control *rc,
			       struct apfs_root *root)
{
	struct apfs_root *reloc_root = root->reloc_root;
	struct apfs_root_item *reloc_root_item;
	int ret;

	/* @root must be a subvolume tree root with a valid reloc tree */
	ASSERT(root->root_key.objectid != APFS_TREE_RELOC_OBJECTID);
	ASSERT(reloc_root);

	reloc_root_item = &reloc_root->root_item;
	memset(&reloc_root_item->drop_progress, 0,
		sizeof(reloc_root_item->drop_progress));
	apfs_set_root_drop_level(reloc_root_item, 0);
	apfs_set_root_refs(reloc_root_item, 0);
	ret = apfs_update_reloc_root(trans, root);
	if (ret)
		return ret;
...
static int clean_dirty_subvols(struct reloc_control *rc)
{
	...
	list_for_each_entry_safe(root, next, &rc->dirty_subvol_roots,
		...
		if (root->root_key.objectid != APFS_TREE_RELOC_OBJECTID) {
			 ...
			 */
			smp_wmb();
			clear_bit(APFS_ROOT_DEAD_RELOC_TREE, &root->state);
			if (reloc_root) {
				/*
				 * apfs_drop_snapshot drops our ref we hold for
				 * ->reloc_root.  If it fails however we must
				 * drop the ref ourselves.
				 */
				ret2 = apfs_drop_snapshot(reloc_root, 0, 1);
				if (ret2 < 0) {
					apfs_put_root(reloc_root);
					if (!ret)
						ret = ret2;
			...
			ret2 = apfs_drop_snapshot(root, 0, 1);
			if (ret2 < 0) {
				apfs_put_root(root);
				if (!ret)
					ret = ret2;
...
static noinline_for_stack int merge_reloc_root(struct reloc_control *rc,
					       struct apfs_root *root)
{
	struct apfs_fs_info *fs_info = rc->extent_root->fs_info;
	struct apfs_key key = {};
	struct apfs_key next_key = {};
	...
	if (apfs_disk_key_objectid(&root_item->drop_progress) == 0) {
		level = apfs_root_level(root_item);
		atomic_inc(&reloc_root->node->refs);
		path->nodes[level] = reloc_root->node;
		path->slots[level] = 0;
	} else {
		apfs_disk_key_to_cpu(root->node, &key, &root_item->drop_progress);

		level = apfs_root_drop_level(root_item);
		BUG_ON(level == 0);
		path->lowest_level = level;
		ret = apfs_search_slot(NULL, reloc_root, &key, path, 0, 0);
		path->lowest_level = 0;
		if (ret < 0) {
			apfs_free_path(path);
			return ret;
		...
		apfs_node_key_to_cpu(path->nodes[level], &next_key,
				      path->slots[level]);
	 ...
	 */
	reserve_level = max_t(int, 1, apfs_root_level(root_item));
	min_reserved = fs_info->nodesize * reserve_level * 2;
	memset(&next_key, 0, sizeof(next_key));

	while (1) {
		ret = apfs_block_rsv_refill(root, rc->block_rsv, min_reserved,
		 ...
		 * depend on apfs_init_reloc_root to update our last_trans.
		 *
		 * But that's ok, we started the trans handle on our
		 * corresponding fs_root, which means it's been added to the
		 * dirty list.  At commit time we'll still call
		 * apfs_update_reloc_root() and update our root item
		 * appropriately.
		 */
		reloc_root->last_trans = trans->transid;
		trans->block_rsv = rc->block_rsv;
		...
		if (!find_next_key(path, level, &key) &&
		    apfs_comp_cpu_keys(path->nodes[level], &next_key, &key) >= 0) {
			ret = 0;
		} else {
		 ...
		 */
		apfs_node_key(path->nodes[level], &root_item->drop_progress,
			       path->slots[level]);
		apfs_set_root_drop_level(root_item, level);

		apfs_end_transaction_throttle(trans);
		trans = NULL;
	 ...
	 */
	leaf = apfs_lock_root_node(root);
	ret = apfs_cow_block(trans, root, leaf, NULL, 0, &leaf,
			      APFS_NESTING_COW);
...
{
	struct apfs_root *root = rc->extent_root;
	struct apfs_fs_info *fs_info = root->fs_info;
	struct apfs_root *reloc_root;
	...
	rc->merging_rsv_size += fs_info->nodesize * (APFS_MAX_LEVEL - 1) * 2;
...
again:
	if (!err) {
		...
		ret = apfs_block_rsv_add(root, rc->block_rsv, num_bytes,
					  APFS_RESERVE_FLUSH_ALL);
		if (ret)
			err = ret;
	}

	trans = apfs_join_transaction(rc->extent_root);
	if (IS_ERR(trans)) {
		if (!err)
			apfs_block_rsv_release(fs_info, rc->block_rsv,
						num_bytes, NULL);
	...
	while (!list_empty(&rc->reloc_roots)) {
		...
		root = apfs_get_fs_root(fs_info, reloc_root->root_key.offset,
				false);
		if (IS_ERR(root)) {
			/*
			...
			apfs_abort_transaction(trans, (int)PTR_ERR(root));
			if (!err)
				err = PTR_ERR(root);
			break;
		}
		...
		/*
		 * set reference count to 1, so apfs_recover_relocation
		 * knows it should resumes merging
		 */
		if (!err)
			apfs_set_root_refs(&reloc_root->root_item, 1);
		ret = apfs_update_reloc_root(trans, root);
...
{
	struct apfs_root *reloc_root, *tmp;
...
again:
	...
	/*
	 * this serializes us with apfs_record_root_in_transaction,
	...
	while (!list_empty(&reloc_roots)) {
		...
		root = apfs_get_fs_root(fs_info, reloc_root->root_key.offset,
					 false);
		if (apfs_root_refs(&reloc_root->root_item) > 0) {
			if (IS_ERR(root)) {
				/*
			...
			if (!IS_ERR(root)) {
				...
				clear_bit(APFS_ROOT_DEAD_RELOC_TREE,
					  &root->state);
				apfs_put_root(root);
			}
...
static int record_reloc_root_in_trans(struct apfs_trans_handle *trans,
				      struct apfs_root *reloc_root)
{
	struct apfs_fs_info *fs_info = reloc_root->fs_info;
	struct apfs_root *root;
	int ret;
	...
	root = apfs_get_fs_root(fs_info, reloc_root->root_key.offset, false);
	...
	ret = apfs_record_root_in_trans(trans, root);
	apfs_put_root(root);
...
static noinline_for_stack
struct apfs_root *select_reloc_root(struct apfs_trans_handle *trans,
				     struct reloc_control *rc,
				     struct apfs_backref_node *node,
				     struct apfs_backref_edge *edges[])
{
	struct apfs_backref_node *next;
	struct apfs_root *root;
	int index = 0;
	int ret;
	...
	"bytenr %llu possibly has multiple roots pointing at the same bytenr %llu",
			...
			apfs_put_root(next->root);
			next->root = apfs_grab_root(root);
			ASSERT(next->root);
	...
	next = node;
	/* setup backref node path for apfs_reloc_cow_block */
	while (1) {
 ...
 */
static noinline_for_stack
struct apfs_root *select_one_root(struct apfs_backref_node *node)
{
	struct apfs_backref_node *next;
	struct apfs_root *root;
	struct apfs_root *fs_root = NULL;
	struct apfs_backref_edge *edges[APFS_MAX_LEVEL - 1];
	int index = 0;
...
u64 calcu_metadata_size(struct reloc_control *rc,
			struct apfs_backref_node *node, int reserve)
{
	struct apfs_fs_info *fs_info = rc->extent_root->fs_info;
	struct apfs_backref_node *next = node;
	struct apfs_backref_edge *edge;
	struct apfs_backref_edge *edges[APFS_MAX_LEVEL - 1];
	u64 num_bytes = 0;
	int index = 0;
...
static int reserve_metadata_space(struct apfs_trans_handle *trans,
				  struct reloc_control *rc,
				  struct apfs_backref_node *node)
{
	struct apfs_root *root = rc->extent_root;
	 ...
	 */
	ret = apfs_block_rsv_refill(root, rc->block_rsv, num_bytes,
				APFS_RESERVE_FLUSH_LIMIT);
	if (ret) {
 ...
 */
static int do_relocation(struct apfs_trans_handle *trans,
			 struct reloc_control *rc,
			 struct apfs_backref_node *node,
			 struct apfs_key *key,
			 struct apfs_path *path, int lowest)
{
	...
	list_for_each_entry(edge, &node->upper, list[LOWER]) {
		...
		if (upper->eb && !upper->locked) {
			if (!lowest) {
				ret = apfs_bin_search(upper->eb, key, &slot);
				if (ret < 0)
					goto next;
				BUG_ON(ret);
				bytenr = apfs_node_blockptr(upper->eb, slot);
				if (node->eb->start == bytenr)
					goto next;
			}
			apfs_backref_drop_node_buffer(upper);
		}
		...
		eb = apfs_read_node_slot(upper->eb, slot);
		if (IS_ERR(eb)) {
			ret = PTR_ERR(eb);
			goto next;
		}
		...
		if (!node->eb) {
			ret = apfs_cow_block(trans, root, eb, upper->eb,
					      slot, &eb, APFS_NESTING_COW);
			apfs_tree_unlock(eb);
			free_extent_buffer(eb);
			if (ret < 0)
				goto next;
			...
			apfs_set_node_blockptr(upper->eb, slot,
						node->eb->start);
			apfs_set_node_ptr_generation(upper->eb, slot,
						      trans->transid);
			apfs_mark_buffer_dirty(upper->eb);

			apfs_init_generic_ref(&ref, APFS_ADD_DELAYED_REF,
					       node->eb->start, blocksize,
					       upper->eb->start);
			ref.real_root = root->root_key.objectid;
			apfs_init_tree_ref(&ref, node->level,
					    apfs_header_owner(upper->eb));
			ret = apfs_inc_extent_ref(trans, &ref);
			if (!ret)
				ret = apfs_drop_subtree(trans, root, eb,
							 upper->eb);
			if (ret)
				apfs_abort_transaction(trans, ret);
		}
next:
		if (!upper->pending)
			...
			apfs_backref_unlock_node_buffer(upper);
		if (ret)
			break;
	}
...
static int link_to_upper(struct apfs_trans_handle *trans,
			 struct reloc_control *rc,
			 struct apfs_backref_node *node,
			 struct apfs_path *path)
{
	...
	apfs_node_key_to_cpu(node->eb, &key, 0);
	return do_relocation(trans, rc, node, &key, path, 0);
}

static int finish_pending_nodes(struct apfs_trans_handle *trans,
				struct reloc_control *rc,
				struct apfs_path *path, int err)
{
	LIST_HEAD(list);
	struct apfs_backref_cache *cache = &rc->backref_cache;
	struct apfs_backref_node *node;
...
static int get_tree_block_key(struct apfs_fs_info *fs_info,
			      struct tree_block *block)
{
	struct extent_buffer *eb;
	...
	if (block->level == 0)
		apfs_item_key_to_cpu(eb, &block->key, 0);
	else
		apfs_node_key_to_cpu(eb, &block->key, 0);
	free_extent_buffer(eb);
	block->key_ready = 1;
	return 0;
}
 ...
 */
static int relocate_tree_block(struct apfs_trans_handle *trans,
				struct reloc_control *rc,
				struct apfs_backref_node *node,
				struct apfs_key *key,
				struct apfs_path *path)
...
out:
	if (ret || node->level == 0 || node->cowonly)
		apfs_backref_cleanup_node(&rc->backref_cache, node);
	return ret;
}
 ...
 */
static noinline_for_stack
int relocate_tree_blocks(struct apfs_trans_handle *trans,
			 struct reloc_control *rc, struct rb_root *blocks)
{
	...
	rbtree_postorder_for_each_entry_safe(block, next, blocks, rb_node) {
		if (!block->key_ready)
			apfs_readahead_tree_block(fs_info, block->bytenr,
						   block->owner, 0,
						   block->level);
	}
				...
				struct file_extent_cluster *cluster)
{
	...
	ret = apfs_alloc_data_chunk_ondemand(inode,
	 ...
	 */
	if (apfs_is_zoned(inode->root->fs_info)) {
	...
	apfs_inode_lock(&inode->vfs_inode, 0);
	for (nr = 0; nr < cluster->nr; nr++) {
		...
		ret = apfs_prealloc_file_range(&inode->vfs_inode, 0, start,
						num_bytes, num_bytes,
						end + 1, &alloc_hint);
		cur_offset = end + 1;
	...
	apfs_inode_unlock(&inode->vfs_inode, 0);

	if (cur_offset < prealloc_end)
		apfs_free_reserved_data_space_noquota(inode->root->fs_info,
			 ...
			 u64 block_start)
{
	...
	while (1) {
		...
		apfs_drop_extent_cache(APFS_I(inode), start, end, 0);
	}
	unlock_extent(&APFS_I(inode)->io_tree, start, end);
	return ret;
}
 ...
 */
noinline int apfs_should_cancel_balance(struct apfs_fs_info *fs_info)
{
	return atomic_read(&fs_info->balance_cancel_req) ||
		atomic_read(&fs_info->reloc_cancel_req) ||
		fatal_signal_pending(current);
}
ALLOW_ERROR_INJECTION(apfs_should_cancel_balance, TRUE);
					...
					struct file_extent_cluster *cluster)
{
	...
	u64 page_start;
	u64 page_end;
	u64 offset = APFS_I(inode)->index_cnt;
	unsigned long index;
	...
	ret = prealloc_file_extent_cluster(APFS_I(inode), cluster);
	if (ret)
		goto out;
	...
	while (index <= last_index) {
		...
		lock_extent(&APFS_I(inode)->io_tree, page_start, page_end);

		if (nr < cluster->nr &&
		    page_start + offset == cluster->boundary[nr]) {
			set_extent_bits(&APFS_I(inode)->io_tree,
					page_start, page_end,
					EXTENT_BOUNDARY);
			nr++;
		}

		ret = apfs_set_extent_delalloc(APFS_I(inode), page_start,
						page_end, 0, NULL);
		if (ret) {
			unlock_page(page);
			put_page(page);
			...
			clear_extent_bits(&APFS_I(inode)->io_tree,
					  page_start, page_end,
		...
		apfs_throttle(fs_info);
		if (apfs_should_cancel_balance(fs_info)) {
			ret = -ECANCELED;
			goto out;
		}
	}
	WARN_ON(nr != cluster->nr);
	if (apfs_is_zoned(fs_info) && !ret)
...
static noinline_for_stack
int relocate_data_extent(struct inode *inode, struct apfs_key *extent_key,
			 struct file_extent_cluster *cluster)
{
	int ret;
			  ...
			  struct rb_root *blocks)
{
	...
	eb =  path->nodes[0];
	item_size = apfs_item_size_nr(eb, path->slots[0]);

	if (extent_key->type == APFS_METADATA_ITEM_KEY ||
	    item_size >= sizeof(*ei) + sizeof(*bi)) {
		unsigned long ptr = 0, end;

		ei = apfs_item_ptr(eb, path->slots[0],
				struct apfs_extent_item);
		end = (unsigned long)ei + item_size;
		if (extent_key->type == APFS_EXTENT_ITEM_KEY) {
			bi = (struct apfs_tree_block_info *)(ei + 1);
			level = apfs_tree_block_level(eb, bi);
			ptr = (unsigned long)(bi + 1);
		} else {
			level = (int)extent_key->offset;
			ptr = (unsigned long)(ei + 1);
		}
		generation = apfs_extent_generation(eb, ei);
		 ...
		 * read_tree_block.
		 */
		if (apfs_extent_refs(eb, ei) == 1 &&
		    !(apfs_extent_flags(eb, ei) &
		      APFS_BLOCK_FLAG_FULL_BACKREF) &&
		    ptr < end) {
			struct apfs_extent_inline_ref *iref;
			int type;

			iref = (struct apfs_extent_inline_ref *)ptr;
			type = apfs_get_extent_inline_ref_type(eb, iref,
							APFS_REF_TYPE_BLOCK);
			if (type == APFS_REF_TYPE_INVALID)
				return -EINVAL;
			if (type == APFS_TREE_BLOCK_REF_KEY)
				owner = apfs_extent_inline_ref_offset(eb, iref);
		}
	} else if (unlikely(item_size == sizeof(struct apfs_extent_item_v0))) {
	...
	if (rb_node)
		apfs_backref_panic(rc->extent_root->fs_info, block->bytenr,
				    -EEXIST);
...
/*
 * helper to add tree blocks for backref of type APFS_SHARED_DATA_REF_KEY
 */
			    ...
			    struct rb_root *blocks)
{
	...
	struct apfs_key key = {};
	int ret;
	bool skinny = apfs_fs_incompat(fs_info, SKINNY_METADATA);
...
again:
	...
	path->skip_locking = 1;
	ret = apfs_search_slot(NULL, rc->extent_root, &key, path, 0, 0);
	if (ret < 0)
		goto out;
...
static int delete_block_group_cache(struct apfs_fs_info *fs_info,
				    struct apfs_block_group *block_group,
				    struct inode *inode,
				    u64 ino)
{
...
truncate:
	...
	ret = apfs_truncate_free_space_cache(trans, block_group, inode);
				 ...
				 u64 data_bytenr)
{
	...
	if (apfs_header_owner(leaf) != APFS_ROOT_TREE_OBJECTID)
		return 0;
	...
	for (i = 0; i < apfs_header_nritems(leaf); i++) {
		...
		if (key.type != APFS_EXTENT_DATA_KEY)
			continue;
		ei = apfs_item_ptr(leaf, i, struct apfs_file_extent_item);
		type = apfs_file_extent_type(leaf, ei);

		if ((type == APFS_FILE_EXTENT_REG ||
		     type == APFS_FILE_EXTENT_PREALLOC) &&
		    apfs_file_extent_disk_bytenr(leaf, ei) == data_bytenr) {
			found = true;
			...
			struct rb_root *blocks)
{
	...
	apfs_release_path(path);
	ret = apfs_find_all_leafs(NULL, fs_info, extent_key->objectid,
				   0, &leaves, NULL, true);
	if (ret < 0)
		return ret;
 ...
 */
static noinline_for_stack
int find_next_extent(struct reloc_control *rc, struct apfs_path *path,
		     struct apfs_key *extent_key)
{
	struct apfs_fs_info *fs_info = rc->extent_root->fs_info;
	struct apfs_key key = {};
	...
	while (1) {
		...
		path->search_commit_root = 1;
		path->skip_locking = 1;
		ret = apfs_search_slot(NULL, rc->extent_root, &key, path,
					0, 0);
		if (ret < 0)
			break;
next:
		...
		if (path->slots[0] >= apfs_header_nritems(leaf)) {
			ret = apfs_next_leaf(rc->extent_root, path);
			if (ret != 0)
				break;
			leaf = path->nodes[0];
		}
...
int prepare_to_relocate(struct reloc_control *rc)
{
	...
	rc->block_rsv = apfs_alloc_block_rsv(rc->extent_root->fs_info,
					      APFS_BLOCK_RSV_TEMP);
	if (!rc->block_rsv)
		return -ENOMEM;
	...
	rc->block_rsv->size = rc->extent_root->fs_info->nodesize *
			      RELOCATION_RESERVED_NODES;
	ret = apfs_block_rsv_refill(rc->extent_root,
...
static noinline_for_stack int relocate_block_group(struct reloc_control *rc)
{
	...
	while (1) {
		...
		progress++;
		trans = apfs_start_transaction(rc->extent_root, 0);
		if (IS_ERR(trans)) {
			err = PTR_ERR(trans);
			trans = NULL;
			break;
...
restart:
		...
		ei = apfs_item_ptr(path->nodes[0], path->slots[0],
				    struct apfs_extent_item);
		flags = apfs_extent_flags(path->nodes[0], ei);
	...
	if (trans && progress && err == -ENOSPC) {
		ret = apfs_force_chunk_alloc(trans, rc->block_group->flags);
		if (ret == 1) {
			err = 0;
			progress = 0;
			goto restart;
	...
	apfs_backref_release_cache(&rc->backref_cache);
	apfs_block_rsv_release(fs_info, rc->block_rsv, (u64)-1, NULL);
...
out_free:
	...
	if (ret < 0 && !err)
		err = ret;
	apfs_free_block_rsv(fs_info, rc->block_rsv);
	apfs_free_path(path);
	return err;
}

static int __insert_orphan_inode(struct apfs_trans_handle *trans,
				 struct apfs_root *root, u64 objectid)
{
	struct apfs_path *path;
	struct apfs_inode_item *item;
	struct extent_buffer *leaf;
	u64 flags = APFS_INODE_NOCOMPRESS | APFS_INODE_PREALLOC;
	int ret;

	if (apfs_is_zoned(trans->fs_info))
		flags &= ~APFS_INODE_PREALLOC;

	path = apfs_alloc_path();
	if (!path)
		return -ENOMEM;

	ret = apfs_insert_empty_inode(trans, root, path, objectid);
	if (ret)
		goto out;

	leaf = path->nodes[0];
	item = apfs_item_ptr(leaf, path->slots[0], struct apfs_inode_item);
	memzero_extent_buffer(leaf, (unsigned long)item, sizeof(*item));
	apfs_set_inode_generation(leaf, item, 1);
	apfs_set_inode_size(leaf, item, 0);
	apfs_set_inode_mode(leaf, item, S_IFREG | 0600);
	apfs_set_inode_flags(leaf, item, flags);
	apfs_mark_buffer_dirty(leaf);
out:
	apfs_free_path(path);
	return ret;
...
static void delete_orphan_inode(struct apfs_trans_handle *trans,
				struct apfs_root *root, u64 objectid)
{
	struct apfs_path *path;
	struct apfs_key key = {};
 ...
 */
static noinline_for_stack
struct inode *create_reloc_inode(struct apfs_fs_info *fs_info,
				 struct apfs_block_group *group)
{
	struct inode *inode = NULL;
	struct apfs_trans_handle *trans;
	struct apfs_root *root;
	...
	root = apfs_grab_root(fs_info->data_reloc_root);
	trans = apfs_start_transaction(root, 6);
	if (IS_ERR(trans)) {
		apfs_put_root(root);
	...
	inode = apfs_iget(fs_info->sb, objectid, root);
	if (IS_ERR(inode)) {
	...
	APFS_I(inode)->index_cnt = group->start;
 ...
 */
static int reloc_chunk_start(struct apfs_fs_info *fs_info)
{
...
"cannot run relocation while send operations are in progress (%d in progress)",
	...
	if (test_and_set_bit(APFS_FS_RELOC_RUNNING, &fs_info->flags)) {
		/* This should not happen */
		spin_unlock(&fs_info->send_reloc_lock);
		apfs_err(fs_info, "reloc already running, cannot start");
		return -EINPROGRESS;
	}
	spin_unlock(&fs_info->send_reloc_lock);

	if (atomic_read(&fs_info->reloc_cancel_req) > 0) {
		apfs_info(fs_info, "chunk relocation canceled on start");
		/*
 ...
 */
static void reloc_chunk_end(struct apfs_fs_info *fs_info)
{
	/* Requested after start, clear bit first so any waiters can continue */
	if (atomic_read(&fs_info->reloc_cancel_req) > 0)
		apfs_info(fs_info, "chunk relocation canceled during operation");
	spin_lock(&fs_info->send_reloc_lock);
	clear_and_wake_up_bit(APFS_FS_RELOC_RUNNING, &fs_info->flags);
	spin_unlock(&fs_info->send_reloc_lock);
	atomic_set(&fs_info->reloc_cancel_req, 0);
}

static struct reloc_control *alloc_reloc_control(struct apfs_fs_info *fs_info)
{
	struct reloc_control *rc;
	...
	apfs_backref_init_cache(fs_info, &rc->backref_cache, 1);
 ...
 */
static void describe_relocation(struct apfs_fs_info *fs_info,
				struct apfs_block_group *block_group)
{
	char buf[128] = {'\0'};

	apfs_describe_block_groups(block_group->flags, buf, sizeof(buf));
...
int apfs_relocate_block_group(struct apfs_fs_info *fs_info, u64 group_start)
{
	...
	bg = apfs_lookup_block_group(fs_info, group_start);
	if (!bg)
		return -ENOENT;

	if (apfs_pinned_by_swapfile(fs_info, bg)) {
	...
	ret = apfs_inc_block_group_ro(rc->block_group, true);
	if (ret) {
		err = ret;
		goto out;
	}
	...
	apfs_wait_block_group_reservations(rc->block_group);
	apfs_wait_nocow_writers(rc->block_group);
	apfs_wait_ordered_roots(fs_info, U64_MAX,
	...
	while (1) {
		 ...
		 * apfs_reloc_cow_block.  Make sure we write everything out
		 * properly so we don't trip over this problem, and then break
		 * out of the loop if we hit an error.
		 */
		if (rc->stage == MOVE_DATA_EXTENTS && rc->found_file_extent) {
			ret = apfs_wait_ordered_range(rc->data_inode, 0,
						       (u64)-1);
			if (ret)
				err = ret;
		...
		apfs_info(fs_info, "found %llu extents, stage: %s",
...
out:
	if (err && rw)
		apfs_dec_block_group_ro(rc->block_group);
	iput(rc->data_inode);
out_put_bg:
	apfs_put_block_group(bg);
...
static noinline_for_stack int mark_garbage_root(struct apfs_root *root)
{
	struct apfs_fs_info *fs_info = root->fs_info;
	struct apfs_trans_handle *trans;
	int ret, err;
	...
	apfs_set_root_refs(&root->root_item, 0);
	ret = apfs_update_root(trans, fs_info->tree_root,
...
int apfs_recover_relocation(struct apfs_root *root)
{
	...
	key.objectid = APFS_TREE_RELOC_OBJECTID;
	key.type = APFS_ROOT_ITEM_KEY;
	key.offset = (u64)-1;

	while (1) {
		ret = apfs_search_slot(NULL, fs_info->tree_root, &key,
					path, 0, 0);
		if (ret < 0) {
			err = ret;
			goto out;
		...
		reloc_root = apfs_read_tree_root(root, &key);
		if (IS_ERR(reloc_root)) {
		...
		if (apfs_root_refs(&reloc_root->root_item) > 0) {
			fs_root = apfs_get_fs_root(fs_info,
	...
	while (!list_empty(&reloc_roots)) {
		...
		fs_root = apfs_get_fs_root(fs_info, reloc_root->root_key.offset,
					    false);
		if (IS_ERR(fs_root)) {
			err = PTR_ERR(fs_root);
		...
		fs_root->reloc_root = apfs_grab_root(reloc_root);
		apfs_put_root(fs_root);
	}
...
out:
	...
	if (err == 0) {
		...
		fs_root = apfs_grab_root(fs_info->data_reloc_root);
		ASSERT(fs_root);
		err = apfs_orphan_cleanup(fs_root);
		apfs_put_root(fs_root);
	}
	return err;
}
...
int apfs_reloc_clone_csums(struct apfs_inode *inode, u64 file_pos, u64 len)
{
	...
	ordered = apfs_lookup_ordered_extent(inode, file_pos);
	BUG_ON(ordered->file_offset != file_pos || ordered->num_bytes != len);

	disk_bytenr = file_pos + inode->index_cnt;
	ret = apfs_lookup_csums_range(fs_info->csum_root, disk_bytenr,
 ...
 *
 * This is similar to apfs_init_reloc_root(), we come out of here with two
			       ...
			       struct apfs_pending_snapshot *pending)
{
	...
	if (rc->merge_reloc_tree) {
		ret = apfs_block_rsv_migrate(&pending->block_rsv,
					      rc->block_rsv,
	...
	new_root->reloc_root = apfs_grab_root(reloc_root);
