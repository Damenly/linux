 ...
 */
static int test_extents(struct apfs_block_group *cache)
{
	int ret = 0;
	...
	ret = apfs_add_free_space(cache, 0, SZ_4M);
	if (ret) {
		test_err("error adding initial extents %d", ret);
		return ret;
	}

	ret = apfs_remove_free_space(cache, 0, SZ_4M);
	if (ret) {
	...
	ret = apfs_remove_free_space(cache, 3 * SZ_1M, SZ_1M);
	if (ret) {
		test_err("error removing tail end %d", ret);
		return ret;
	}

	ret = apfs_remove_free_space(cache, 0, SZ_1M);
	if (ret) {
		test_err("error removing front end %d", ret);
		return ret;
	}

	ret = apfs_remove_free_space(cache, SZ_2M, 4096);
	if (ret) {
...
static int test_bitmaps(struct apfs_block_group *cache, u32 sectorsize)
{
	...
	ret = apfs_remove_free_space(cache, SZ_1M, SZ_2M);
	if (ret) {
	...
	ret = apfs_remove_free_space(cache, next_bitmap_offset - SZ_1M, SZ_2M);
	if (ret) {
...
static int test_bitmaps_and_extents(struct apfs_block_group *cache,
				    u32 sectorsize)
{
	...
	ret = apfs_remove_free_space(cache, SZ_4M, SZ_1M);
	if (ret) {
	...
	ret = apfs_remove_free_space(cache, SZ_512K, 3 * SZ_1M);
	if (ret) {
		test_err("couldn't remove overlapping space %d", ret);
		return ret;
	}
	...
	ret = apfs_remove_free_space(cache, 3 * SZ_1M, SZ_4M);
	if (ret) {
	...
	ret = apfs_remove_free_space(cache, bitmap_offset + SZ_1M, 5 * SZ_1M);
	if (ret) {
	 ...
	 * to return -EAGAIN back from apfs_remove_extent, make sure this
	 * doesn't happen.
	 */
	...
	ret = apfs_remove_free_space(cache, SZ_1M, 3 * SZ_1M);
	if (ret) {
...
static bool test_use_bitmap(struct apfs_free_space_ctl *ctl,
...
/* Used by test_steal_space_from_bitmap_to_extent(). */
static int
check_num_extents_and_bitmaps(const struct apfs_block_group *cache,
...
/* Used by test_steal_space_from_bitmap_to_extent(). */
static int check_cache_empty(struct apfs_block_group *cache)
{
	u64 offset;
	u64 max_extent_size;
	...
	offset = apfs_find_space_for_alloc(cache, 0, 4096, 0,
					    &max_extent_size);
	if (offset != 0) {
 ...
 */
static int
test_steal_space_from_bitmap_to_extent(struct apfs_block_group *cache,
				       u32 sectorsize)
{
	int ret;
	u64 offset;
	u64 max_extent_size;
	const struct apfs_free_space_op test_free_space_ops = {
		.use_bitmap = test_use_bitmap,
	};
	const struct apfs_free_space_op *orig_free_space_ops;
	 ...
	 */
	ret = apfs_remove_free_space(cache,
				      SZ_128M + 768 * SZ_1K,
				      SZ_128M - 768 * SZ_1K);
	if (ret) {
	 ...
	 */
	ret = apfs_add_free_space(cache, SZ_128M, SZ_512K);
	if (ret) {
	 ...
	 */
	ret = apfs_add_free_space(cache, SZ_128M + SZ_16M, sectorsize);
	if (ret) {
		test_err("error adding free space: %d", ret);
		return ret;
	}
	 ...
	 */
	ret = apfs_add_free_space(cache, SZ_128M - SZ_128K, SZ_128K);
	if (ret) {
		test_err("error adding free space: %d", ret);
		return ret;
	}
	...
	offset = apfs_find_space_for_alloc(cache,
					    0, SZ_1M, 0,
					    &max_extent_size);
	 ...
	 */
	ret = apfs_remove_free_space(cache, 0, SZ_128M - 768 * SZ_1K);
	if (ret) {
		test_err("failed to free part of bitmap space %d", ret);
		return ret;
	}
	 ...
	 * and doesn't insert a new extent entry to represent this region.
	 */
	ret = apfs_add_free_space(cache, SZ_128M - SZ_512K, SZ_512K);
	if (ret) {
		test_err("error adding free space: %d", ret);
		return ret;
	}
	 ...
	 * steal this space region.
	 */
	ret = apfs_add_free_space(cache, SZ_32M, 2 * sectorsize);
	if (ret) {
		test_err("error adding free space: %d", ret);
		return ret;
	}
	 ...
	 */
	ret = apfs_add_free_space(cache, SZ_128M, SZ_128K);
	if (ret) {
		test_err("error adding free space: %d", ret);
		return ret;
	}
	...
	offset = apfs_find_space_for_alloc(cache, 0, SZ_1M, 0,
					    &max_extent_size);
...
int apfs_test_free_space_cache(u32 sectorsize, u32 nodesize)
{
	...
	test_msg("running apfs free space cache tests");
	 ...
	 */
	cache = apfs_alloc_dummy_block_group(fs_info,
