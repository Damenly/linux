 ...
 * are entering apfs_get_extent() concurrently, t1 is reading [8K, 16K), t2 is
 * reading [0, 8K)
 *
 *     t1                            t2
 *  apfs_get_extent()              apfs_get_extent()
 ...
 *                                    -> #handle -EEXIST
 */
static int test_case_1(struct apfs_fs_info *fs_info,
		struct extent_map_tree *em_tree)
{
	...
	em->block_len = len;
	write_lock(&em_tree->lock);
	ret = apfs_add_extent_mapping(fs_info, em_tree, &em, em->start, em->len);
	write_unlock(&em_tree->lock);
	if (ret) {
 ...
 */
static int test_case_2(struct apfs_fs_info *fs_info,
		struct extent_map_tree *em_tree)
{
	struct extent_map *em;
	int ret;
...
static int __test_case_3(struct apfs_fs_info *fs_info,
 ...
 *
 *         t1                       t2
 *  cow_file_range()	     apfs_get_extent()
 *                            -> lookup_extent_mapping()
 *   -> add_extent_mapping()
 *                            -> add_extent_mapping()
 */
static int test_case_3(struct apfs_fs_info *fs_info,
		struct extent_map_tree *em_tree)
{
	int ret;
...
static int __test_case_4(struct apfs_fs_info *fs_info,
		struct extent_map_tree *em_tree, u64 start)
{
	struct extent_map *em;
	u64 len = SZ_4K;
 ...
 *
 *         t1                                t2
 *  apfs_get_blocks_direct()	       apfs_get_blocks_direct()
 *   -> apfs_get_extent()              -> apfs_get_extent()
 *       -> lookup_extent_mapping()
 *       -> add_extent_mapping()            -> lookup_extent_mapping()
 *          # load [0, 32K)
 *   -> apfs_new_extent_direct()
 *       -> apfs_drop_extent_cache()
 *          # split [0, 32K)
 *       -> add_extent_mapping()
 *          # add [8K, 32K)
 *                                          -> add_extent_mapping()
 ...
 *                                             # [0, 32K)
 */
static int test_case_4(struct apfs_fs_info *fs_info,
		struct extent_map_tree *em_tree)
{
	int ret;
...
static int test_rmap_block(struct apfs_fs_info *fs_info,
			   struct rmap_test_vector *test)
{
	...
	for (i = 0; i < map->num_stripes; i++) {
		struct apfs_device *dev = apfs_alloc_dummy_device(fs_info);
	...
	ret = apfs_rmap_block(fs_info, em->start, NULL, apfs_sb_offset(1),
	...
	if (out_stripe_len != APFS_STRIPE_LEN) {
...
int apfs_test_extent_map(void)
{
	...
	struct rmap_test_vector rmap_tests[] = {
		{
			 ...
			 */
			.raid_type = APFS_BLOCK_GROUP_RAID1,
	 ...
	 */
	fs_info = apfs_alloc_dummy_fs_info(PAGE_SIZE, PAGE_SIZE);
	if (!fs_info) {
