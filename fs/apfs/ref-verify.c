...
static void __print_stack_trace(struct apfs_fs_info *fs_info,
				struct ref_action *ra)
{
	if (ra->trace_len == 0) {
		apfs_err(fs_info, "  ref-verify: no stacktrace");
		return;
	}
...
static inline void __print_stack_trace(struct apfs_fs_info *fs_info,
				       struct ref_action *ra)
{
	apfs_err(fs_info, "  ref-verify: no stacktrace support");
}
#endif
...
static struct block_entry *add_block_entry(struct apfs_fs_info *fs_info,
					   u64 bytenr, u64 len,
					   u64 root_objectid)
{
...
static int add_tree_block(struct apfs_fs_info *fs_info, u64 ref_root,
...
static int add_shared_data_ref(struct apfs_fs_info *fs_info,
			       ...
			       u64 num_bytes)
{
	...
	if (insert_ref_entry(&be->refs, ref)) {
		spin_unlock(&fs_info->ref_verify_lock);
		apfs_err(fs_info, "existing shared ref when reading from disk?");
		kfree(ref);
		return -EINVAL;
	}
	spin_unlock(&fs_info->ref_verify_lock);
...
static int add_extent_data_ref(struct apfs_fs_info *fs_info,
			       ...
			       u64 bytenr, u64 num_bytes)
{
	...
	struct ref_entry *ref;
	struct root_entry *re;
	u64 ref_root = apfs_extent_data_ref_root(leaf, dref);
	u64 owner = apfs_extent_data_ref_objectid(leaf, dref);
	u64 offset = apfs_extent_data_ref_offset(leaf, dref);
	u32 num_refs = apfs_extent_data_ref_count(leaf, dref);
	...
	if (insert_ref_entry(&be->refs, ref)) {
		spin_unlock(&fs_info->ref_verify_lock);
		apfs_err(fs_info, "existing ref when reading from disk?");
		kfree(ref);
		return -EINVAL;
	}
	...
	if (!re) {
		spin_unlock(&fs_info->ref_verify_lock);
		apfs_err(fs_info, "missing root in new block entry?");
		return -EINVAL;
	}
...
static int process_extent_item(struct apfs_fs_info *fs_info,
			       struct apfs_path *path, struct apfs_key *key,
			       int slot, int *tree_block_level)
{
	...
	if ((key->type == APFS_EXTENT_ITEM_KEY) &&
		...
		*tree_block_level = apfs_tree_block_level(leaf, info);
	...
	while (ptr < end) {
		...
		type = apfs_extent_inline_ref_type(leaf, iref);
		...
		case APFS_SHARED_DATA_REF_KEY:
			...
			count = apfs_shared_data_ref_count(leaf, sref);
			ret = add_shared_data_ref(fs_info, offset, count,
						  key->objectid, key->offset);
			break;
		default:
			apfs_err(fs_info, "invalid key type in iref");
			ret = -EINVAL;
			break;
		}
		if (ret)
...
static int process_leaf(struct apfs_root *root,
			struct apfs_path *path, u64 *bytenr, u64 *num_bytes,
...
static int walk_down_tree(struct apfs_root *root, struct apfs_path *path,
			  ...
			  int *tree_block_level)
{
	...
	while (level >= 0) {
		if (level) {
			eb = apfs_read_node_slot(path->nodes[level],
						  path->slots[level]);
			if (IS_ERR(eb))
				return PTR_ERR(eb);
...
static int walk_up_tree(struct apfs_path *path, int *level)
{
	int l;

	for (l = 0; l < APFS_MAX_LEVEL; l++) {
		if (!path->nodes[l])
			continue;
		if (l) {
			path->slots[l]++;
			if (path->slots[l] <
			    apfs_header_nritems(path->nodes[l])) {
				*level = l;
				return 0;
			}
		}
		apfs_tree_unlock_rw(path->nodes[l], path->locks[l]);
...
static void dump_ref_action(struct apfs_fs_info *fs_info,
			    struct ref_action *ra)
{
	apfs_err(fs_info,
 ...
 * awesome.
 */
static void dump_block_entry(struct apfs_fs_info *fs_info,
			     struct block_entry *be)
{
	struct ref_entry *ref;
	struct root_entry *re;
...
"  ref root %llu, parent %llu, owner %llu, offset %llu, num_refs %llu",
	...
	for (n = rb_first(&be->roots); n; n = rb_next(n)) {
		re = rb_entry(n, struct root_entry, node);
		apfs_err(fs_info, "  root entry %llu, num_refs %llu",
...
/*
 * apfs_ref_tree_mod: called when we modify a ref for a bytenr
 *
		       ...
		       struct apfs_ref *generic_ref)
{
	...
	if (!apfs_test_opt(fs_info, REF_VERIFY))
		return 0;
	...
	metadata = owner < APFS_FIRST_FREE_OBJECTID;
	...
	ref->root_objectid = ref_root;
	ref->offset = offset;
	ref->num_refs = (action == APFS_DROP_DELAYED_REF) ? -1 : 1;
...
"trying to do action %d to bytenr %llu num_bytes %llu but there is no existing entry!",
	...
	exist = insert_ref_entry(&be->refs, ref);
	if (exist) {
		if (action == APFS_DROP_DELAYED_REF) {
			if (exist->num_refs == 0) {
				apfs_err(fs_info,
...
"dropping a ref for a root that doesn't have a ref on the block");
	...
	if (!parent && !re) {
		...
		if (!re) {
			 ...
			 */
			apfs_err(fs_info, "failed to find root %llu for %llu",
	...
	if (action == APFS_DROP_DELAYED_REF) {
		if (re)
			re->num_refs--;
		be->num_refs--;
	} else if (action == APFS_ADD_DELAYED_REF) {
		be->num_refs++;
		if (re)
			re->num_refs++;
	}
...
out:
	if (ret)
		apfs_clear_opt(fs_info->mount_opt, REF_VERIFY);
	return ret;
}
...
int apfs_build_ref_tree(struct apfs_fs_info *fs_info)
{
	...
	eb = apfs_read_lock_root_node(fs_info->extent_root);
