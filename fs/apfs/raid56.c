...
static int alloc_rbio_parity_pages(struct apfs_raid_bio *rbio)
{
	int i;
	struct page *page;
 ...
 */
static int rbio_add_io_page(struct apfs_raid_bio *rbio,
			    struct bio_list *bio_list,
			    struct page *page,
			    int stripe_nr,
			    ...
			    unsigned long bio_max_len)
{
	...
	bio = apfs_io_bio_alloc(bio_max_len >> PAGE_SHIFT ?: 1);
	apfs_io_bio(bio)->device = stripe->dev;
 ...
 */
static void validate_rbio_for_rmw(struct apfs_raid_bio *rbio)
{
...
static void index_rbio_pages(struct apfs_raid_bio *rbio)
{
	...
	bio_list_for_each(bio, &rbio->bio_list) {
		...
		if (bio_flagged(bio, BIO_CLONED))
			bio->bi_iter = apfs_io_bio(bio)->iter;
...
static noinline void finish_rmw(struct apfs_raid_bio *rbio)
{
	struct apfs_bio *bbio = rbio->bbio;
 ...
 */
static int find_bio_stripe(struct apfs_raid_bio *rbio,
			   struct bio *bio)
{
 ...
 */
static int find_logical_bio_stripe(struct apfs_raid_bio *rbio,
				   struct bio *bio)
{
 ...
 * bio.
 */
static int fail_bio_stripe(struct apfs_raid_bio *rbio,
			   struct bio *bio)
{
 ...
 */
static int raid56_rmw_stripe(struct apfs_raid_bio *rbio)
{
	int bios_to_read = 0;
	struct bio_list bio_list;
	int ret;
	...
	while ((bio = bio_list_pop(&bio_list))) {
		...
		apfs_bio_wq_end_io(rbio->fs_info, bio, APFS_WQ_ENDIO_RAID56);
 ...
 */
static int full_stripe_write(struct apfs_raid_bio *rbio)
{
	int ret;
 ...
 */
static int partial_stripe_write(struct apfs_raid_bio *rbio)
{
	int ret;
 ...
 */
static int __raid56_parity_write(struct apfs_raid_bio *rbio)
{
...
{
	struct apfs_raid_bio *ra = container_of(a, struct apfs_raid_bio,
						 plug_list);
	struct apfs_raid_bio *rb = container_of(b, struct apfs_raid_bio,
						 plug_list);
...
static void run_plug(struct apfs_plug_cb *plug)
{
	struct apfs_raid_bio *cur;
	struct apfs_raid_bio *last = NULL;
	...
	while (!list_empty(&plug->rbio_list)) {
		cur = list_entry(plug->rbio_list.next,
				 struct apfs_raid_bio, plug_list);
 ...
 */
static void unplug_work(struct apfs_work *work)
{
	struct apfs_plug_cb *plug;
	plug = container_of(work, struct apfs_plug_cb, work);
	run_plug(plug);
}

static void apfs_raid_unplug(struct blk_plug_cb *cb, bool from_schedule)
{
	struct apfs_plug_cb *plug;
	plug = container_of(cb, struct apfs_plug_cb, cb);

	if (from_schedule) {
		apfs_init_work(&plug->work, unplug_work, NULL, NULL);
		apfs_queue_work(plug->info->rmw_workers,
				 &plug->work);
		return;
	}
	run_plug(plug);
			...
			struct apfs_bio *bbio, u64 stripe_len)
{
	...
	rbio->operation = APFS_RBIO_WRITE;
	...
	cb = blk_check_plugged(apfs_raid_unplug, fs_info, sizeof(*plug));
	if (cb) {
		plug = container_of(cb, struct apfs_plug_cb, cb);
		if (!plug->info) {
			plug->info = fs_info;
 ...
 */
static void __raid_recover_end_io(struct apfs_raid_bio *rbio)
{
	int pagenr, stripe;
	void **pointers;
	void **unmap_array;
	...
	if (rbio->operation == APFS_RBIO_READ_REBUILD ||
	    rbio->operation == APFS_RBIO_REBUILD_MISSING) {
		spin_lock_irq(&rbio->bio_list_lock);
		set_bit(RBIO_RMW_LOCKED_BIT, &rbio->flags);
		spin_unlock_irq(&rbio->bio_list_lock);
	}
	...
	for (pagenr = 0; pagenr < rbio->stripe_npages; pagenr++) {
		 ...
		 */
		if (rbio->operation == APFS_RBIO_PARITY_SCRUB &&
		...
		if (rbio->bbio->map_type & APFS_BLOCK_GROUP_RAID6) {
			/*
...
pstripe:
		 ...
		 */
		if (rbio->operation == APFS_RBIO_WRITE) {
...
cleanup_io:
	...
	if (rbio->operation == APFS_RBIO_READ_REBUILD ||
		...
		if (rbio->operation == APFS_RBIO_WRITE)
			finish_rmw(rbio);
		else if (rbio->operation == APFS_RBIO_PARITY_SCRUB)
			  ...
			  int mirror_num, int generic_io)
{
	...
	if (generic_io) {
		...
		apfs_io_bio(bio)->mirror_num = mirror_num;
	}
	...
	rbio->operation = APFS_RBIO_READ_REBUILD;
	bio_list_add(&rbio->bio_list, bio);
	rbio->bio_list_bytes = bio->bi_iter.bi_size;
...
static void rmw_work(struct apfs_work *work)
{
	...
	rbio = container_of(work, struct apfs_raid_bio, work);
			       ...
			       unsigned long *dbitmap, int stripe_nsectors)
{
	...
	rbio->operation = APFS_RBIO_PARITY_SCRUB;

	/*
	 * After mapping bbio with APFS_MAP_WRITE, parities have been sorted
 ...
 */
static int alloc_rbio_essential_pages(struct apfs_raid_bio *rbio)
{
	int i;
	int bit;
	int index;
...
static inline int is_data_stripe(struct apfs_raid_bio *rbio, int stripe)
{
 ...
 * stripes
 */
static void validate_rbio_for_parity_scrub(struct apfs_raid_bio *rbio)
{
	if (atomic_read(&rbio->error) > rbio->bbio->max_errors)
		goto cleanup;
...
static void raid56_parity_scrub_stripe(struct apfs_raid_bio *rbio)
{
	int bios_to_read = 0;
	struct bio_list bio_list;
	int ret;
